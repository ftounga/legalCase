...
// SF-222-04 : composant simulateur assistance éducative (F-FA-ASSISTANCE-EDUCATIVE).
import { AssistanceEducativeSectionComponent } from '../assistance-educative-section/assistance-educative-section.component';
...
export class DecisionToolsPanelComponent implements OnInit, OnChanges {
  ...
  static readonly TOOL_REGISTRY: ReadonlyMap<string, DecisionToolRegistryEntry> =
    new Map<string, DecisionToolRegistryEntry>([
      ...
      // SF-222-02 : composant simulateur complet (POST/GET backend SF-222-02).
      // Outil P3 famille FR — TGD téléphone grave danger éligibilité (art. 41-3-1 CPP).
      // Anti-doublon F-FA-14 : analyzer d'éligibilité, distinct de la mesure TGD de
      // l'ordonnance de protection. Pré-fill IA branché sur `synthesis.familleExtractedData`.
      ['F-FA-TGD', {
      ...
      // SF-222-04 : composant simulateur complet (POST/GET backend SF-222-04).
      // Outil P3 famille FR — assistance éducative, mineur en danger (art. 375 et s. Cciv).
      // Invariant « 1 situation = 1 outil » : un seul outil oriente AED / AEMO /
      // OPP-placement / pas de mesure. Pré-fill IA branché sur `synthesis.familleExtractedData`.
      ['F-FA-ASSISTANCE-EDUCATIVE', {
        displayLabel: 'Assistance éducative — enfant en danger (FR)',
        component: AssistanceEducativeSectionComponent,
        inputs: (ctx) => ({
          caseFileId: ctx.caseFileId,
          workspaceCountry: ctx.workspaceCountry,
          aiData: ctx.synthesis?.familleExtractedData,
      ...
      // art. 343-1 et s. — à vérifier). Outil multi-vues unique (PLENIERE /
      // SIMPLE / CO_PARENTALE). Migration 581 — CONTEXTUAL, NOUVEAU flag pivot
      // `adoption_be_detectee` (consolidé dans Sf223Detail). Pré-fill IA F-246 :
      // type + agrément. Backend SF-223-02 bundle.
      ['adoption-be', {
        displayLabel: 'Adoption — recevabilité (Belgique)',
        component: AdoptionBeSectionComponent,
        inputs: (ctx) => ({
          caseFileId: ctx.caseFileId,
          workspaceCountry: ctx.workspaceCountry,
          aiData: ctx.synthesis?.familleExtractedData,
      ...
      // SF-223-08 — reconnaissance / exequatur d'une décision familiale étrangère
      // BE (CDIP art. 22-27 ; art. 21 Const. / CC 161 mariage religieux non-civil).
      // Migration 593 — CONTEXTUAL, NOUVEAU flag pivot
      // `dip_reconnaissance_decision_be_detectee` (Sf223Detail). Pré-fill IA
      // F-246 : nature + pays + date (sous-objet
      // `dip_reconnaissance_decision_be_detection`). Backend SF-223-08 bundle.
      // DISTINCT de dip-be-loi-applicable-famille (SF-223-07) et de
      // mariage-etranger-be-reconnaissance (F-217).
      ['dip-be-reconnaissance-decision-etrangere', {
        displayLabel: 'Reconnaissance / exequatur d\'une décision familiale étrangère (Belgique)',
        component: DipBeReconnaissanceDecisionEtrangereSectionComponent,
        inputs: (ctx) => ({
          caseFileId: ctx.caseFileId,
          workspaceCountry: ctx.workspaceCountry,
          aiData: ctx.synthesis?.familleExtractedData,
  ...
  static readonly THEME_BY_TOOL_ID: ReadonlyMap<string, ThemeKey> = new Map<string, ThemeKey>([
    ...
    // SF-222-04 : assistance éducative — mineur en danger (FR, art. 375 et s. Cciv).
    ['F-FA-ASSISTANCE-EDUCATIVE', 'DIAGNOSTIC'],
  ...
  /**
   * SF-268-02 — état d'ouverture du Catalogue (section des outils à activation
   * manuelle). **Replié par défaut** : c'est le désencombrement demandé par le
   * PO — la longue liste d'outils complémentaires ne charge plus l'écran et ne
   * s'affiche, organisée par thème, qu'à la demande.
   */
  readonly catalogExpanded = signal(false);

  /**
   * SF-268-02 — regroupe les outils du **Catalogue** (`visibility().catalog`,
   * activation manuelle) par thème métier (`THEME_BY_TOOL_ID`), dans l'ordre
   * canonique `THEMES_ORDERED`. Un thème sans outil au catalogue n'apparaît pas.
   * Un toolId sans mapping tombe dans `DIAGNOSTIC` (fallback identique à
   * `themedTools()`). Pure présentation : aucun changement de visibilité.
   */
  themedCatalog(): { key: ThemeKey; label: string; toolIds: string[] }[] {
    const catalog = this.visibility()?.catalog ?? [];
    const byTheme = new Map<ThemeKey, string[]>();
    ...
    const result: { key: ThemeKey; label: string; toolIds: string[] }[] = [];
    for (const theme of this.themesOrdered) {
      const ids = byTheme.get(theme.key);
      if (ids && ids.length > 0) {
        result.push({ key: theme.key, label: theme.label, toolIds: ids });
      }
    }
    return result;
  }
  ...
  private loadVisibility(showSpinner: boolean): void {
    ...
    this.caseFileService.getDecisionToolsVisibility(this.caseFileId).subscribe({
      next: (result) => {
        ...
        // SF-268-02 — la visibilité a changé : on replie le Catalogue pour
        // repartir d'un écran désencombré (la liste d'outils complémentaires a
        // pu changer).
        this.catalogExpanded.set(false);
