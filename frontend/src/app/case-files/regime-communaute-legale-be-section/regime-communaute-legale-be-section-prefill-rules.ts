/**
 * F-217 SF-217-03 — Helper partagé pour l'outil "Régime de communauté légale
 ...
 * pivot dédié à la composition du patrimoine (origine des biens, financement,
 * nature des dettes). Aucun champ du formulaire n'est donc pré-rempli depuis
 * l'analyse — `computePrefillCount` retourne 0 de manière inconditionnelle.
 * C'est un état factuel documenté (pas une dette masquée, cf. SF-217-00) :
 * l'extension IA fera l'objet d'une SF ultérieure si le signal terrain émerge.
 */
...
export function computePrefillCount(
  _input: RegimeCommunauteLegaleBePrefillInput,
): number {
  return 0;
}
