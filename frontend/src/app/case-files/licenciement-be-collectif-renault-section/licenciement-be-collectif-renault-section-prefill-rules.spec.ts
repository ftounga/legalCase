import {
  LicenciementBeCollectifRenaultSectionPrefillRules as R,
} from './licenciement-be-collectif-renault-section-prefill-rules';

/**
 * SF-219-07b — tests unitaires du helper pur.
 *
 ...
 * effectifEtp, salaireBrutMensuel).</p>
 *
 ...
 */
describe('LicenciementBeCollectifRenaultSectionPrefillRules', () => {
    ...
    it('BELGIQUE avec champs Travail BE riches → 0 (V1)', () => {
      expect(R.computePrefillCount({
        ...
        aiData: {
          ...
          motifRupture: 'licenciement collectif',
          anneesCarriereSalarie: 18,
          salaireBrutMensuel: 3500,
        },
      })).toBe(0);
    });

    it('BELGIQUE avec motifRupture=licenciement_collectif → 0', () => {
      expect(R.computePrefillCount({
        workspaceCountry: 'BELGIQUE',
        aiData: { motifRupture: 'licenciement collectif' },
      })).toBe(0);
    });
