import {
  RccBeEntrepriseDifficulteSectionPrefillRules as R,
} from './rcc-be-entreprise-difficulte-section-prefill-rules';

/**
 * SF-219-03b — tests unitaires du helper pur.
 *
 ...
 * de champs IA Travail BE riches (dateNaissanceSalarie, dateRuptureContrat,
 * motifRupture, anneesCarriereSalarie, salaireBrutMensuel,
 * salaireBrutAnnuel).</p>
 *
 ...
 */
describe('RccBeEntrepriseDifficulteSectionPrefillRules', () => {
  ...
  describe('V1 — invariant : count toujours 0', () => {
    ...
    it('BELGIQUE avec dateNaissanceSalarie + dateRuptureContrat → 0 (pas extractible V1)', () => {
      expect(R.computePrefillCount({
        workspaceCountry: 'BELGIQUE',
        aiData: {
          dateNaissanceSalarie: '1970-03-15',
    ...
    it('BELGIQUE avec motifRupture=licenciement → 0', () => {
      expect(R.computePrefillCount({
        workspaceCountry: 'BELGIQUE',
        aiData: { motifRupture: 'licenciement' },
      })).toBe(0);
    });

    it('BELGIQUE avec salaireBrutMensuel → 0 (brut ≠ net RCC)', () => {
      expect(R.computePrefillCount({
        workspaceCountry: 'BELGIQUE',
    ...
    it('workspaceCountry absent → 0', () => {
      expect(R.computePrefillCount({
        aiData: { dateRuptureContrat: '2025-12-31' },
      })).toBe(0);
    });
  });
  ...
  describe('Contrat F-236 — comportement déterministe', () => {
    ...
    it('plusieurs invocations → 0 stable', () => {
      for (let i = 0; i < 5; i++) {
        expect(R.computePrefillCount({})).toBe(0);
      }
    });
