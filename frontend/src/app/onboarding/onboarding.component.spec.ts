...
describe('OnboardingComponent', () => {
  ...
  // T-01 : soumission avec nom valide → appel POST + redirect /case-files
  it('soumission avec nom valide → createWorkspace appelé + redirect vers /case-files', fakeAsync(() => {
    ...
    expect(workspaceService.createWorkspace).toHaveBeenCalledWith('Cabinet Martin');
    expect(router.navigate).toHaveBeenCalledWith(['/case-files']);
  }));

  // T-02 : soumission avec nom vide → erreur inline, pas d'appel HTTP
