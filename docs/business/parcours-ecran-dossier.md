...
| Décision | **Stratégie de dossier** (coiffe du tableau de bord, pas un bloc primaire ; **carte compacte repliable** depuis F-295 — résumé par défaut, dépli du détail in situ) | `app-case-strategy` | Reco stratégique LLM en lecture des verdicts calculés + synthèse (F-286) |
...
| 2026-06-15 | F-295 / SF-295-00b (cadrage écran — re-design carte stratégie compacte repliable) | **15ᵉ passage**. Verdict GO avec ajustements. **Re-design de présentation** (pas de workflow → étape 0 cohérence fonctionnelle exemptée) de la carte **« Stratégie de dossier »** (`app-case-strategy`, coiffe `decision-space__verdict`) : remplacer le **pavé markdown affiché en permanence** (`[innerHTML]`, jugé encombrant au test 2026-06-15) par une **carte compacte repliable** — résumé en tête par défaut (voie / posture / priorité des chefs, lignes ou chips) + bouton **« Voir le détail »** qui déplie le markdown complet **in situ**. **Aucun déplacement, aucun bloc primaire nouveau, frontend pur** (F-286 génération/stockage inchangé, 0 migration, 0 LLM). Réduit la **charge** de la coiffe (le tableau de bord remonte dans le champ de vision) et **renforce la lisibilité** de la séquence stratégie → tableau de bord. **Points durs / invariants** : repli ≠ suppression (détail toujours dépliable) ; pied de carte (date + « Rédiger mes conclusions » + « Régénérer ») **hors zone repliable**, visible replié/déplié ; état par défaut **replié** ; résumé = vue condensée de la reco existante (pas de nouvelle info) ; pas de 4ᵉ couleur ; responsive < 1024/640 px préservé ; états vides F-258 / génération inchangés. Cf. `docs/features/F-295/SF-295-00b-ux-coherence.md`. |
| 2026-06-13 | F-288 / SF-288-00b (cadrage écran — écran de composition des conclusions avant génération) | **14ᵉ passage**. Verdict GO avec ajustements. Sur la **page Conclusions** (`/case-files/:id/conclusions`, F-267 ; `conclusions-section`), le clic **« Générer »** (état `NOT_GENERATED`) **et** « Régénérer » (stale) **ouvre d'abord un modal de composition** au lieu de lancer directement : l'avocat y **coche/décoche, par dimension, ce qu'il verse dans l'acte** (toutes cochées par défaut) puis **« Confirmer & générer »**. **Audit des ingrédients** : 5 des 7 ingrédients de l'acte sont **déjà curés ailleurs** (pièces F-260, pistes RETAINED, jurisprudence F-242, jurisprudence adverse SF-98-56, synthèse) → hors périmètre ; **scope = les 3 trous** en **vagues** : vague 1 **outils décisionnels** (`detectAll` injecte tout), vague 2 **chefs de demande** (fondation F-262 non branchée), vague 3 **moyens adverses** (auto-extraits, pas de tri). **Aucune nouvelle route, aucun 4ᵉ bloc primaire** (modal par-dessus `/conclusions`). **Points durs** : (a) **charge** — modal **à la demande** (ne charge jamais l'état `NOT_GENERATED`), listes compactes repliables par dimension (pattern F-268) ; (b) **distinct de l'encart F-258** qui reste en amont dans `NOT_GENERATED` (alerte des non calculés ≠ composition des calculés) ; (c) **continuité** — choix **durable**, le modal se ré-ouvre **pré-coché** en régénération (F-271/F-278) sans reset ; (d) 0 élément curable → pas de modal (générer direct) ; tout décoché → avertissement non bloquant. Invariant « 1 outil = 1 situation » intact : F-288 **filtre l'entrée du prompt**, n'écrit sur aucune table `*_analysis`/`decision_tool_visibility_rules`, ne déplace/masque aucun outil. Cf. `docs/features/F-288/SF-288-00-coherence.md` + `SF-288-00b-ux-coherence.md`. |
