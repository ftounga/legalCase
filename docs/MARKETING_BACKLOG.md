...
| M-73 | One-pager traction commerciale présentable | Haute | `Couvert par F-187` | **2026-05-04** : décision de basculer cette tâche en feature produit `F-187 Export traction commerciale (one-pager PDF auto)` côté super-admin (cf. PRODUCT_SPEC.md). Friction Canva manuelle mensuelle (10-15 min) éliminée par génération PDF auto avec formulaire pour les inputs variables (verbatims, partenaires, accroche) + lecture automatique des KPIs F-76. Time-box 1 j de dev, 2 SF parallèles (SF-187-01 backend + SF-187-02 frontend, skill `parallel-frontback-delivery.md`). Statut M-73 → repassera `Terminé` quand F-187 sera Terminée et déployée en staging. |
