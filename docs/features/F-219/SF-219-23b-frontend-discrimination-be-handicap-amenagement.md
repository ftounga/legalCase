# Mini-spec — F-219 / SF-219-23b-frontend Outil discrimination BE handicap amenagement (UI)

## Identifiant

`F-219 / SF-219-23b-frontend`
...
`feat/SF-219-23b-frontend-discrimination-be-handicap-amenagement`
...
Livrer la section frontend décisionnelle de l'outil **discrimination BE handicap amenagement** BE (consommant SF-219-23 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 141** (pattern F-213).

---

## Contrat API (figé en SF-219-23 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/discrimination-be-handicap-amenagement`
- `GET` du même path
- Body : voir SF-219-23-backend (inputs métier spécifiques).
...
`discrimination-be-handicap-amenagement-section.component` — formulaire + verdict :
- Champs : selon SF-219-23-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `discrimination-be-handicap-amenagement-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `discrimination-be-handicap-amenagement`
- Visibility : `ALWAYS_ON`, `priority = 141`
- Ordre : position 41 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-discrimination-be-handicap-amenagement-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='discrimination-be-handicap-amenagement'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=141`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"discrimination-be-handicap-amenagement"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"discrimination-be-handicap-amenagement"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 141.
...
- [ ] `discrimination-be-handicap-amenagement-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `discrimination-be-handicap-amenagement-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/discrimination-be-handicap-amenagement-section/discrimination-be-handicap-amenagement-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/discrimination-be-handicap-amenagement-section/discrimination-be-handicap-amenagement-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/discrimination-be-handicap-amenagement.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-discrimination-be-handicap-amenagement-visibility.xml`
...
- SF-219-23 backend — doit être mergée avant le push de cette SF.
