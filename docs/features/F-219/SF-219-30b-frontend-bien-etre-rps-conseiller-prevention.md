# Mini-spec — F-219 / SF-219-30b-frontend Outil bien etre rps conseiller prevention (UI)

## Identifiant

`F-219 / SF-219-30b-frontend`
...
`feat/SF-219-30b-frontend-bien-etre-rps-conseiller-prevention`
...
Livrer la section frontend décisionnelle de l'outil **bien etre rps conseiller prevention** BE (consommant SF-219-30 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 148** (pattern F-213).

---

## Contrat API (figé en SF-219-30 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/bien-etre-rps-conseiller-prevention`
- `GET` du même path
- Body : voir SF-219-30-backend (inputs métier spécifiques).
...
`bien-etre-rps-conseiller-prevention-section.component` — formulaire + verdict :
- Champs : selon SF-219-30-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `bien-etre-rps-conseiller-prevention-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `bien-etre-rps-conseiller-prevention`
- Visibility : `ALWAYS_ON`, `priority = 148`
- Ordre : position 48 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-bien-etre-rps-conseiller-prevention-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='bien-etre-rps-conseiller-prevention'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=148`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"bien-etre-rps-conseiller-prevention"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"bien-etre-rps-conseiller-prevention"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 148.
...
- [ ] `bien-etre-rps-conseiller-prevention-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `bien-etre-rps-conseiller-prevention-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/bien-etre-rps-conseiller-prevention-section/bien-etre-rps-conseiller-prevention-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/bien-etre-rps-conseiller-prevention-section/bien-etre-rps-conseiller-prevention-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/bien-etre-rps-conseiller-prevention.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-bien-etre-rps-conseiller-prevention-visibility.xml`
...
- SF-219-30 backend — doit être mergée avant le push de cette SF.
