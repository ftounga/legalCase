# Mini-spec — F-219 / SF-219-04b-frontend Outil cumul rcc allocations (UI)

## Identifiant

`F-219 / SF-219-04b-frontend`
...
`feat/SF-219-04b-frontend-cumul-rcc-allocations`
...
Livrer la section frontend décisionnelle de l'outil **cumul rcc allocations** BE (consommant SF-219-04 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 122** (pattern F-213).

---

## Contrat API (figé en SF-219-04 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/cumul-rcc-allocations`
- `GET` du même path
- Body : voir SF-219-04-backend (inputs métier spécifiques).
...
`cumul-rcc-allocations-section.component` — formulaire + verdict :
- Champs : selon SF-219-04-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `cumul-rcc-allocations-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `cumul-rcc-allocations`
- Visibility : `ALWAYS_ON`, `priority = 122`
- Ordre : position 22 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-cumul-rcc-allocations-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='cumul-rcc-allocations'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=122`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"cumul-rcc-allocations"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"cumul-rcc-allocations"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 122.
...
- [ ] `cumul-rcc-allocations-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `cumul-rcc-allocations-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/cumul-rcc-allocations-section/cumul-rcc-allocations-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/cumul-rcc-allocations-section/cumul-rcc-allocations-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/cumul-rcc-allocations.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-cumul-rcc-allocations-visibility.xml`
...
- SF-219-04 backend — doit être mergée avant le push de cette SF.
