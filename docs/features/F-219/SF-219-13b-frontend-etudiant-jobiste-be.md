# Mini-spec — F-219 / SF-219-13b-frontend Outil etudiant jobiste be (UI)

## Identifiant

`F-219 / SF-219-13b-frontend`
...
`feat/SF-219-13b-frontend-etudiant-jobiste-be`
...
Livrer la section frontend décisionnelle de l'outil **etudiant jobiste be** BE (consommant SF-219-13 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 131** (pattern F-213).

---

## Contrat API (figé en SF-219-13 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/etudiant-jobiste-be`
- `GET` du même path
- Body : voir SF-219-13-backend (inputs métier spécifiques).
...
`etudiant-jobiste-be-section.component` — formulaire + verdict :
- Champs : selon SF-219-13-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `etudiant-jobiste-be-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `etudiant-jobiste-be`
- Visibility : `ALWAYS_ON`, `priority = 131`
- Ordre : position 31 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-etudiant-jobiste-be-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='etudiant-jobiste-be'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=131`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"etudiant-jobiste-be"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"etudiant-jobiste-be"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 131.
...
- [ ] `etudiant-jobiste-be-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `etudiant-jobiste-be-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/etudiant-jobiste-be-section/etudiant-jobiste-be-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/etudiant-jobiste-be-section/etudiant-jobiste-be-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/etudiant-jobiste-be.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-etudiant-jobiste-be-visibility.xml`
...
- SF-219-13 backend — doit être mergée avant le push de cette SF.
