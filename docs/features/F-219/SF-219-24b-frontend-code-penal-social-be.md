# Mini-spec — F-219 / SF-219-24b-frontend Outil code penal social be (UI)

## Identifiant

`F-219 / SF-219-24b-frontend`
...
`feat/SF-219-24b-frontend-code-penal-social-be`
...
Livrer la section frontend décisionnelle de l'outil **code penal social be** BE (consommant SF-219-24 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 142** (pattern F-213).

---

## Contrat API (figé en SF-219-24 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/code-penal-social-be`
- `GET` du même path
- Body : voir SF-219-24-backend (inputs métier spécifiques).
...
`code-penal-social-be-section.component` — formulaire + verdict :
- Champs : selon SF-219-24-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `code-penal-social-be-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `code-penal-social-be`
- Visibility : `ALWAYS_ON`, `priority = 142`
- Ordre : position 42 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-code-penal-social-be-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='code-penal-social-be'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=142`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"code-penal-social-be"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"code-penal-social-be"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 142.
...
- [ ] `code-penal-social-be-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `code-penal-social-be-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/code-penal-social-be-section/code-penal-social-be-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/code-penal-social-be-section/code-penal-social-be-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/code-penal-social-be.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-code-penal-social-be-visibility.xml`
...
- SF-219-24 backend — doit être mergée avant le push de cette SF.
