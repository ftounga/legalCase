# Mini-spec — F-219 / SF-219-11b-frontend Outil conge education paye region (UI)

## Identifiant

`F-219 / SF-219-11b-frontend`
...
`feat/SF-219-11b-frontend-conge-education-paye-region`
...
Livrer la section frontend décisionnelle de l'outil **conge education paye region** BE (consommant SF-219-11 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 129** (pattern F-213).

---

## Contrat API (figé en SF-219-11 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/conge-education-paye-region`
- `GET` du même path
- Body : voir SF-219-11-backend (inputs métier spécifiques).
...
`conge-education-paye-region-section.component` — formulaire + verdict :
- Champs : selon SF-219-11-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `conge-education-paye-region-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `conge-education-paye-region`
- Visibility : `ALWAYS_ON`, `priority = 129`
- Ordre : position 29 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-conge-education-paye-region-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='conge-education-paye-region'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=129`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"conge-education-paye-region"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"conge-education-paye-region"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 129.
...
- [ ] `conge-education-paye-region-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `conge-education-paye-region-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/conge-education-paye-region-section/conge-education-paye-region-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/conge-education-paye-region-section/conge-education-paye-region-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/conge-education-paye-region.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-conge-education-paye-region-visibility.xml`
...
- SF-219-11 backend — doit être mergée avant le push de cette SF.
