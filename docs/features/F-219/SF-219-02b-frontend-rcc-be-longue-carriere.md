# Mini-spec — F-219 / SF-219-02b-frontend Outil rcc BE longue carriere (UI)

## Identifiant

`F-219 / SF-219-02b-frontend`
...
`feat/SF-219-02b-frontend-rcc-be-longue-carriere`
...
Livrer la section frontend décisionnelle de l'outil **rcc BE longue carriere** BE (consommant SF-219-02 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 120** (pattern F-213).

---

## Contrat API (figé en SF-219-02 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/rcc-be-longue-carriere`
- `GET` du même path
- Body : voir SF-219-02-backend (inputs métier spécifiques).
...
`rcc-be-longue-carriere-section.component` — formulaire + verdict :
- Champs : selon SF-219-02-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `rcc-be-longue-carriere-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `rcc-be-longue-carriere`
- Visibility : `ALWAYS_ON`, `priority = 120`
- Ordre : position 20 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-rcc-be-longue-carriere-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='rcc-be-longue-carriere'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=120`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"rcc-be-longue-carriere"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"rcc-be-longue-carriere"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 120.
...
- [ ] `rcc-be-longue-carriere-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `rcc-be-longue-carriere-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/rcc-be-longue-carriere-section/rcc-be-longue-carriere-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/rcc-be-longue-carriere-section/rcc-be-longue-carriere-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/rcc-be-longue-carriere.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-rcc-be-longue-carriere-visibility.xml`
...
- SF-219-02 backend — doit être mergée avant le push de cette SF.
