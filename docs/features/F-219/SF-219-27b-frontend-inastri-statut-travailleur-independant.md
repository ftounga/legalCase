# Mini-spec — F-219 / SF-219-27b-frontend Outil inastri statut travailleur independant (UI)

## Identifiant

`F-219 / SF-219-27b-frontend`
...
`feat/SF-219-27b-frontend-inastri-statut-travailleur-independant`
...
Livrer la section frontend décisionnelle de l'outil **inastri statut travailleur independant** BE (consommant SF-219-27 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 145** (pattern F-213).

---

## Contrat API (figé en SF-219-27 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/inastri-statut-travailleur-independant`
- `GET` du même path
- Body : voir SF-219-27-backend (inputs métier spécifiques).
...
`inastri-statut-travailleur-independant-section.component` — formulaire + verdict :
- Champs : selon SF-219-27-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `inastri-statut-travailleur-independant-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `inastri-statut-travailleur-independant`
- Visibility : `ALWAYS_ON`, `priority = 145`
- Ordre : position 45 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
- THEME_BY_TOOL_ID : thème adapté au type d'outil (legal-decisional-analyzer)

### Visibility seed (migration backend incluse dans cette SF)

Migration `XXX-add-inastri-statut-travailleur-independant-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='inastri-statut-travailleur-independant'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=145`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"inastri-statut-travailleur-independant"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"inastri-statut-travailleur-independant"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 145.
...
- [ ] `inastri-statut-travailleur-independant-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `inastri-statut-travailleur-independant-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/inastri-statut-travailleur-independant-section/inastri-statut-travailleur-independant-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/inastri-statut-travailleur-independant-section/inastri-statut-travailleur-independant-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/inastri-statut-travailleur-independant.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-inastri-statut-travailleur-independant-visibility.xml`
...
- SF-219-27 backend — doit être mergée avant le push de cette SF.
