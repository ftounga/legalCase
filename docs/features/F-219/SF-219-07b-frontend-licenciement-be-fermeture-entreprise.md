# Mini-spec — F-219 / SF-219-07b-frontend Outil licenciement BE fermeture entreprise (UI)

## Identifiant

`F-219 / SF-219-07b-frontend`
...
`feat/SF-219-07b-frontend-licenciement-be-fermeture-entreprise`
...
Livrer la section frontend décisionnelle de l'outil **licenciement BE fermeture entreprise** BE (consommant SF-219-07 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 125** (pattern F-213).

---

## Contrat API (figé en SF-219-07 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/licenciement-be-fermeture-entreprise`
- `GET` du même path
- Body : voir SF-219-07-backend (inputs métier spécifiques).
...
`licenciement-be-fermeture-entreprise-section.component` — formulaire + verdict :
- Champs : selon SF-219-07-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `licenciement-be-fermeture-entreprise-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `licenciement-be-fermeture-entreprise`
- Visibility : `ALWAYS_ON`, `priority = 125`
- Ordre : position 25 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-licenciement-be-fermeture-entreprise-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='licenciement-be-fermeture-entreprise'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=125`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"licenciement-be-fermeture-entreprise"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"licenciement-be-fermeture-entreprise"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 125.
...
- [ ] `licenciement-be-fermeture-entreprise-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `licenciement-be-fermeture-entreprise-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-licenciement-be-fermeture-entreprise-visibility.xml`
...
- SF-219-07 backend — doit être mergée avant le push de cette SF.
