# Mini-spec — F-219 / SF-219-19b-frontend Outil droit deconnexion be (UI)

## Identifiant

`F-219 / SF-219-19b-frontend`
...
`feat/SF-219-19b-frontend-droit-deconnexion-be`
...
Livrer la section frontend décisionnelle de l'outil **droit deconnexion be** BE (consommant SF-219-19 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 137** (pattern F-213).

---

## Contrat API (figé en SF-219-19 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/droit-deconnexion-be`
- `GET` du même path
- Body : voir SF-219-19-backend (inputs métier spécifiques).
...
`droit-deconnexion-be-section.component` — formulaire + verdict :
- Champs : selon SF-219-19-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `droit-deconnexion-be-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `droit-deconnexion-be`
- Visibility : `ALWAYS_ON`, `priority = 137`
- Ordre : position 37 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-droit-deconnexion-be-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='droit-deconnexion-be'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=137`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"droit-deconnexion-be"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"droit-deconnexion-be"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 137.
...
- [ ] `droit-deconnexion-be-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `droit-deconnexion-be-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/droit-deconnexion-be-section/droit-deconnexion-be-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/droit-deconnexion-be-section/droit-deconnexion-be-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/droit-deconnexion-be.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-droit-deconnexion-be-visibility.xml`
...
- SF-219-19 backend — doit être mergée avant le push de cette SF.
