# Mini-spec — F-219 / SF-219-18b-frontend Outil semaine 4 jours be (UI)

## Identifiant

`F-219 / SF-219-18b-frontend`
...
`feat/SF-219-18b-frontend-semaine-4-jours-be`
...
Livrer la section frontend décisionnelle de l'outil **semaine 4 jours be** BE (consommant SF-219-18 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 136** (pattern F-213).

---

## Contrat API (figé en SF-219-18 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/semaine-4-jours-be`
- `GET` du même path
- Body : voir SF-219-18-backend (inputs métier spécifiques).
...
`semaine-4-jours-be-section.component` — formulaire + verdict :
- Champs : selon SF-219-18-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `semaine-4-jours-be-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `semaine-4-jours-be`
- Visibility : `ALWAYS_ON`, `priority = 136`
- Ordre : position 36 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-semaine-4-jours-be-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='semaine-4-jours-be'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=136`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"semaine-4-jours-be"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"semaine-4-jours-be"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 136.
...
- [ ] `semaine-4-jours-be-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `semaine-4-jours-be-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/semaine-4-jours-be-section/semaine-4-jours-be-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/semaine-4-jours-be-section/semaine-4-jours-be-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/semaine-4-jours-be.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-semaine-4-jours-be-visibility.xml`
...
- SF-219-18 backend — doit être mergée avant le push de cette SF.
