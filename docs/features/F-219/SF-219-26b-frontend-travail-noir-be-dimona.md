# Mini-spec — F-219 / SF-219-26b-frontend Outil travail noir BE dimona (UI)

## Identifiant

`F-219 / SF-219-26b-frontend`
...
`feat/SF-219-26b-frontend-travail-noir-be-dimona`
...
Livrer la section frontend décisionnelle de l'outil **travail noir BE dimona** BE (consommant SF-219-26 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 144** (pattern F-213).

---

## Contrat API (figé en SF-219-26 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/travail-noir-be-dimona`
- `GET` du même path
- Body : voir SF-219-26-backend (inputs métier spécifiques).
...
`travail-noir-be-dimona-section.component` — formulaire + verdict :
- Champs : selon SF-219-26-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `travail-noir-be-dimona-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `travail-noir-be-dimona`
- Visibility : `ALWAYS_ON`, `priority = 144`
- Ordre : position 44 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-travail-noir-be-dimona-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='travail-noir-be-dimona'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=144`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"travail-noir-be-dimona"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"travail-noir-be-dimona"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 144.
...
- [ ] `travail-noir-be-dimona-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `travail-noir-be-dimona-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/travail-noir-be-dimona-section/travail-noir-be-dimona-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/travail-noir-be-dimona-section/travail-noir-be-dimona-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/travail-noir-be-dimona.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-travail-noir-be-dimona-visibility.xml`
...
- SF-219-26 backend — doit être mergée avant le push de cette SF.
