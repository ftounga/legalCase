# Mini-spec — F-219 / SF-219-16b-frontend Outil teletravail BE cct 85 149 (UI)

## Identifiant

`F-219 / SF-219-16b-frontend`
...
`feat/SF-219-16b-frontend-teletravail-be-cct-85-149`
...
Livrer la section frontend décisionnelle de l'outil **teletravail BE cct 85 149** BE (consommant SF-219-16 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 134** (pattern F-213).

---

## Contrat API (figé en SF-219-16 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/teletravail-be-cct-85-149`
- `GET` du même path
- Body : voir SF-219-16-backend (inputs métier spécifiques).
...
`teletravail-be-cct-85-149-section.component` — formulaire + verdict :
- Champs : selon SF-219-16-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `teletravail-be-cct-85-149-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `teletravail-be-cct-85-149`
- Visibility : `ALWAYS_ON`, `priority = 134`
- Ordre : position 34 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-teletravail-be-cct-85-149-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='teletravail-be-cct-85-149'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=134`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"teletravail-be-cct-85-149"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"teletravail-be-cct-85-149"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 134.
...
- [ ] `teletravail-be-cct-85-149-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `teletravail-be-cct-85-149-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/teletravail-be-cct-85-149-section/teletravail-be-cct-85-149-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/teletravail-be-cct-85-149-section/teletravail-be-cct-85-149-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/teletravail-be-cct-85-149.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-teletravail-be-cct-85-149-visibility.xml`
...
- SF-219-16 backend — doit être mergée avant le push de cette SF.
