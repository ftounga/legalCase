# Mini-spec — F-219 / SF-219-05b-frontend Outil outplacement BE general 30sem (UI)

## Identifiant

`F-219 / SF-219-05b-frontend`
...
`feat/SF-219-05b-frontend-outplacement-be-general-30sem`
...
Livrer la section frontend décisionnelle de l'outil **outplacement BE general 30sem** BE (consommant SF-219-05 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 123** (pattern F-213).

---

## Contrat API (figé en SF-219-05 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/outplacement-be-general-30sem`
- `GET` du même path
- Body : voir SF-219-05-backend (inputs métier spécifiques).
...
`outplacement-be-general-30sem-section.component` — formulaire + verdict :
- Champs : selon SF-219-05-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `outplacement-be-general-30sem-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `outplacement-be-general-30sem`
- Visibility : `ALWAYS_ON`, `priority = 123`
- Ordre : position 23 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-outplacement-be-general-30sem-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='outplacement-be-general-30sem'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=123`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"outplacement-be-general-30sem"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"outplacement-be-general-30sem"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 123.
...
- [ ] `outplacement-be-general-30sem-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `outplacement-be-general-30sem-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/outplacement-be-general-30sem-section/outplacement-be-general-30sem-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/outplacement-be-general-30sem-section/outplacement-be-general-30sem-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/outplacement-be-general-30sem.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-outplacement-be-general-30sem-visibility.xml`
...
- SF-219-05 backend — doit être mergée avant le push de cette SF.
