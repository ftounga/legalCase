# Mini-spec — F-219 / SF-219-28b-frontend Outil mp fedris reconnaissance (UI)

## Identifiant

`F-219 / SF-219-28b-frontend`
...
`feat/SF-219-28b-frontend-mp-fedris-reconnaissance`
...
Livrer la section frontend décisionnelle de l'outil **mp fedris reconnaissance** BE (consommant SF-219-28 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 146** (pattern F-213).

---

## Contrat API (figé en SF-219-28 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/mp-fedris-reconnaissance`
- `GET` du même path
- Body : voir SF-219-28-backend (inputs métier spécifiques).
...
`mp-fedris-reconnaissance-section.component` — formulaire + verdict :
- Champs : selon SF-219-28-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `mp-fedris-reconnaissance-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `mp-fedris-reconnaissance`
- Visibility : `ALWAYS_ON`, `priority = 146`
- Ordre : position 46 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-mp-fedris-reconnaissance-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='mp-fedris-reconnaissance'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=146`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"mp-fedris-reconnaissance"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"mp-fedris-reconnaissance"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 146.
...
- [ ] `mp-fedris-reconnaissance-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `mp-fedris-reconnaissance-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/mp-fedris-reconnaissance-section/mp-fedris-reconnaissance-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/mp-fedris-reconnaissance-section/mp-fedris-reconnaissance-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/mp-fedris-reconnaissance.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-mp-fedris-reconnaissance-visibility.xml`
...
- SF-219-28 backend — doit être mergée avant le push de cette SF.
