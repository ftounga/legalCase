# Mini-spec — F-219 / SF-219-20b-frontend Outil pecule vacances be (UI)

## Identifiant

`F-219 / SF-219-20b-frontend`
...
`feat/SF-219-20b-frontend-pecule-vacances-be`
...
Livrer la section frontend décisionnelle de l'outil **pecule vacances be** BE (consommant SF-219-20 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 138** (pattern F-213).

---

## Contrat API (figé en SF-219-20 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/pecule-vacances-be`
- `GET` du même path
- Body : voir SF-219-20-backend (inputs métier spécifiques).
...
`pecule-vacances-be-section.component` — formulaire + verdict :
- Champs : selon SF-219-20-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `pecule-vacances-be-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `pecule-vacances-be`
- Visibility : `ALWAYS_ON`, `priority = 138`
- Ordre : position 38 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-pecule-vacances-be-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='pecule-vacances-be'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=138`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"pecule-vacances-be"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"pecule-vacances-be"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 138.
...
- [ ] `pecule-vacances-be-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `pecule-vacances-be-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/pecule-vacances-be-section/pecule-vacances-be-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/pecule-vacances-be-section/pecule-vacances-be-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/pecule-vacances-be.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-pecule-vacances-be-visibility.xml`
...
- SF-219-20 backend — doit être mergée avant le push de cette SF.
