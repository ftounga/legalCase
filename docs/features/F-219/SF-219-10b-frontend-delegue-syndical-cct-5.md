# Mini-spec — F-219 / SF-219-10b-frontend Outil delegue syndical cct 5 (UI)

## Identifiant

`F-219 / SF-219-10b-frontend`
...
`feat/SF-219-10b-frontend-delegue-syndical-cct-5`
...
Livrer la section frontend décisionnelle de l'outil **delegue syndical cct 5** BE (consommant SF-219-10 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 128** (pattern F-213).

---

## Contrat API (figé en SF-219-10 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/delegue-syndical-cct-5`
- `GET` du même path
- Body : voir SF-219-10-backend (inputs métier spécifiques).
...
`delegue-syndical-cct-5-section.component` — formulaire + verdict :
- Champs : selon SF-219-10-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `delegue-syndical-cct-5-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `delegue-syndical-cct-5`
- Visibility : `ALWAYS_ON`, `priority = 128`
- Ordre : position 28 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-delegue-syndical-cct-5-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='delegue-syndical-cct-5'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=128`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"delegue-syndical-cct-5"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"delegue-syndical-cct-5"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 128.
...
- [ ] `delegue-syndical-cct-5-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `delegue-syndical-cct-5-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/delegue-syndical-cct-5-section/delegue-syndical-cct-5-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/delegue-syndical-cct-5-section/delegue-syndical-cct-5-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/delegue-syndical-cct-5.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-delegue-syndical-cct-5-visibility.xml`
...
- SF-219-10 backend — doit être mergée avant le push de cette SF.
