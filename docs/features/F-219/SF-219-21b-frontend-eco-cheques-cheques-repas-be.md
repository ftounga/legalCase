# Mini-spec — F-219 / SF-219-21b-frontend Outil eco cheques cheques repas be (UI)

## Identifiant

`F-219 / SF-219-21b-frontend`
...
`feat/SF-219-21b-frontend-eco-cheques-cheques-repas-be`
...
Livrer la section frontend décisionnelle de l'outil **eco cheques cheques repas be** BE (consommant SF-219-21 backend). Conforme au pattern canonique F-IA-04, exposée **uniquement aux workspaces BELGIQUE / DROIT_DU_TRAVAIL**, visibility **ALWAYS_ON priority 139** (pattern F-213).

---

## Contrat API (figé en SF-219-21 backend)

- `POST /api/v1/case-files/{caseFileId}/decision-tools/eco-cheques-cheques-repas-be`
- `GET` du même path
- Body : voir SF-219-21-backend (inputs métier spécifiques).
...
`eco-cheques-cheques-repas-be-section.component` — formulaire + verdict :
- Champs : selon SF-219-21-backend (formulaire reactive Forms, inputs typés).
...
Limité aux champs cross-domain déjà extraits par le modèle BE de base. Détail dans `eco-cheques-cheques-repas-be-section-prefill-rules.ts`. `getPrefillCount(input)` retourne le nombre de champs pré-fillés.

### Entrée TOOL_REGISTRY

- `tool_id` : `eco-cheques-cheques-repas-be`
- Visibility : `ALWAYS_ON`, `priority = 139`
- Ordre : position 39 dans la séquence TOOL_REGISTRY BE (cf. étape 0 bis)
...
Migration `XXX-add-eco-cheques-cheques-repas-be-visibility.xml` :
- INSERT `decision_tool_visibility_rules` : `tool_id='eco-cheques-cheques-repas-be'`, `country='BELGIQUE'`, `legal_domain='DROIT_DU_TRAVAIL'`, `visibility='ALWAYS_ON'`, `priority=139`.

### KNOWN_NO_DASHBOARD_TILE_IDS

**Obligatoire** (mémoire `feedback_f213_backend_pattern` — récidive 2 fois en 24 h sur F-213) :
- Ajouter `"eco-cheques-cheques-repas-be"` à `DashboardTileToolIdIntegrityIT.KNOWN_NO_DASHBOARD_TILE_IDS` côté backend.
- Sinon master-red garanti.

### KNOWN_FRONTEND_TOOL_IDS

Ajouter `"eco-cheques-cheques-repas-be"` à `KNOWN_FRONTEND_TOOL_IDS` (`DecisionToolVisibilityIntegrityIT` côté backend) — sinon panel orphelin (mémoire `feedback_pre_merge_visibility_seed_check`).
...
- [ ] Migration visibility ALWAYS_ON / BELGIQUE / DROIT_DU_TRAVAIL appliquée avec priority 139.
...
- [ ] `eco-cheques-cheques-repas-be-section-prefill-rules.spec.ts` — tests pré-fill (0 / N champs, mapping valeurs).
- [ ] `eco-cheques-cheques-repas-be-section.component.spec.ts` — rendu sans BELGIQUE, pré-fill, verdict positif/négatif, refresh, snackbar erreur, OnPush markForCheck.
...
- `frontend/src/app/case-files/eco-cheques-cheques-repas-be-section/eco-cheques-cheques-repas-be-section.component.{ts,html,scss,spec.ts}`
- `frontend/src/app/case-files/eco-cheques-cheques-repas-be-section/eco-cheques-cheques-repas-be-section-prefill-rules.{ts,spec.ts}`
- `frontend/src/app/core/models/eco-cheques-cheques-repas-be.model.ts`
...
- `backend/src/main/resources/db/changelog/migrations/XXX-add-eco-cheques-cheques-repas-be-visibility.xml`
...
- SF-219-21 backend — doit être mergée avant le push de cette SF.
