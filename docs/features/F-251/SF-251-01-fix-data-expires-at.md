...
- [x] Test d'intégration `SubscriptionsBackfillExpiresAtIT` qui couvre :
  - FREE + `started_at` non NULL (2026-05-01) + `expires_at = NULL` → patché à `2026-05-15`
  - FREE + `expires_at` déjà fixée (`2027-01-01`) → inchangée
  - SOLO + `expires_at = NULL` → inchangée (plan payant hors scope)
  - Idempotence : 2ᵉ exécution retourne 0 row affectée
  - Cas `started_at NULL` non testable au runtime (colonne NOT NULL en schéma) — le `COALESCE` reste défensif, sémantique applicative équivalente couverte par `SubscriptionPrePersistTest.freeWithNullStartedAt_setsBothNowAndExpiresAt` (SF-251-02)
