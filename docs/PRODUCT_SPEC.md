...
| ID | Feature | Cible | Notes |
|----|---------|-------|-------|
| F-110 | Guides & barèmes métier par domaine | V4 — **À spécifier** | Écran "Guides & barèmes" accessible depuis le menu, adapté par domaine juridique (Travail / Immigration / Famille). Quatre volets : (0) **Prérequis technique (SF backend)** — migration des données hardcodées Java (`LitigationTypeMapper`, `CompensationCalculator`, `ImmigrationPieceReferentiel`, `ImmigrationProcedureReferentiel`, `PensionAlimentaireCalculator`, `PrestationCompensatoireCalculator`) vers table `legal_referentials` en base + enrichissement avec les procédures manquantes (dont `REGULARISATION_EXCEPTIONNELLE` immigration — pièces + jalons France/Belgique) ; (1) **Consultation** — simulateurs et référentiels en lecture seule par domaine (barème Macron, délais prescription, grille UNAF/CGKR, prestation compensatoire, checklist pièces immigration avec régularisation) ; (2) **Personnalisation workspace** — l'avocat OWNER peut ajouter des pièces custom à la checklist immigration par-dessus le référentiel officiel, les barèmes légaux restent en lecture seule ; (3) **Mise à jour IA automatique** — cron configurable (défaut 6 mois) déclenche une vérification Claude via web search (Légifrance, UNAF, JO). Si l'IA détecte une divergence, une notification est envoyée au OWNER avec les nouvelles valeurs proposées. Le OWNER confirme ou refuse avant application en base. |
