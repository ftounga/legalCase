...
| ID | Feature | Cible | Notes |
|----|---------|-------|-------|
| F-112 | Amélioration UX barres de progression | V4 — **En cours** | Refonte des indicateurs de progression sur la page dossier : (1) upload avec vrai % par fichier via `reportProgress`; (2) modes de barre adaptatifs (buffer PENDING, determinate PROCESSING/DONE, warn FAILED); (3) compteurs humains "processedItems / totalItems"; (4) pipeline visuel linéaire Upload → Analyse docs → Synthèse → Questions → Synthèse enrichie; (5) temps écoulé sur le job actif. Pure frontend — aucun endpoint nouveau. |
| F-111 | Tableau de bord opérationnel workspace | V4 — **En cours** | Page `/dashboard` affichée à la connexion (remplace liste dossiers par défaut). Dossiers ouverts + statut, délais urgents J+7 tous dossiers, alertes checklist en retard (NON_COMPLIANT > 7j), activité récente (5 dernières analyses). SF-111-01 mergée 2026-04-04 : GET /api/v1/dashboard, DashboardService, 612 tests verts. SF-111-02 en cours : DashboardComponent Angular. |
