...
| F-54 | Comparaison inter-analyses (diff) | V2 | Permettre à l'avocat de comparer deux versions de synthèse (STANDARD ou ENRICHED) sur un même dossier. Diff sémantique item par item sur tous les champs (faits, points juridiques, risques, questions ouvertes, timeline). Choix libre des deux versions à comparer. Ajouts en vert, suppressions en rouge, inchangé en gris. Disponible dès que le dossier a au moins 2 versions d'analyse. |
