...
| F-111 | Tableau de bord opérationnel workspace | V4 — **Backlog** | Page `/dashboard` affichée à la connexion (remplace liste dossiers par défaut). Dossiers ouverts + statut (en attente analyse / analyse en cours / à compléter), délais urgents J-7 et J-3 tous dossiers, alertes checklist en retard (NON_COMPLIANT > 7j sans action), activité récente (dernières analyses, derniers uploads). Pure frontend — agrège les endpoints existants. |
