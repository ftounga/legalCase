...
| F-56 | Diff sémantique avec attribution des raisons | V2 | Amélioration de F-54. Remplacement du diff exact par un diff sémantique via Haiku (4 états : ajouté/supprimé/inchangé/enrichi-bleu). Snapshot des documents par analyse via table `analysis_documents`. Attribution de la raison par item : document ajouté/supprimé pour STANDARD vs STANDARD, réponse Q&R pour STANDARD vs ENRICHIE. Tooltip au survol des items colorés. Cache du résultat sémantique par paire (fromId, toId). |
