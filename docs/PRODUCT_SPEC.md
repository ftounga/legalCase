...
| F-117 | SEO — Mots-clés métier dans meta tags + structured data JSON-LD | V5 — **Backlog** | Enrichir title et meta description avec les mots-clés métier (avocat, droit du travail, contentieux, analyse IA). Ajouter un bloc JSON-LD schema.org SoftwareApplication sur la landing pour les rich snippets Google. Complète F-75 (meta tags) et F-116 (prerendering). |
