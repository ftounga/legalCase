...
| ID | Feature | Cible | Notes |
|----|---------|-------|-------|
| F-110 | Guides & barèmes métier par domaine | V4 — **À spécifier** | Écran "Guides & barèmes" accessible depuis le menu, adapté par domaine juridique (Travail / Immigration / Famille). Trois volets : (1) **Consultation** — simulateurs et référentiels en lecture seule (barème Macron, délais prescription, grille UNAF/CGKR, prestation compensatoire, checklist pièces immigration) ; (2) **Personnalisation workspace** — l'avocat OWNER peut ajouter des pièces custom à la checklist immigration par-dessus le référentiel officiel ; (3) **Mise à jour IA automatique** — cron configurable (défaut 6 mois) déclenche une vérification Claude via web search (Légifrance, UNAF, JO). Si l'IA détecte une divergence entre les valeurs en vigueur et celles stockées, une notification est envoyée au OWNER avec les nouvelles valeurs proposées. Le OWNER confirme ou refuse avant application. Les barèmes légaux (Macron, UNAF, délais) sont stockés en base (table `legal_referentials`) pour être mis à jour sans release. |
