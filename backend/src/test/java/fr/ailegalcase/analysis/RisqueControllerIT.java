...
/**
 * F-195 SF-195-01 — IT du contrôleur risques markables.
 *
 * <p>Pattern miroir {@link PieceManquanteControllerIT} (F-194).</p>
 */
...
class RisqueControllerIT {
    ...
    void setUp() {
        ...
        User u1 = new User(); u1.setEmail("rsq-" + ts + "@ex.com"); u1.setStatus("ACTIVE");
        u1 = userRepository.save(u1);
        AuthAccount a1 = new AuthAccount(); a1.setUser(u1); a1.setProvider("GOOGLE");
        a1.setProviderUserId("g-rsq-" + ts);
        authAccountRepository.save(a1);
        Workspace ws1 = new Workspace();
        ws1.setName("W1-" + ts); ws1.setSlug("ws-rsq-1-" + ts); ws1.setOwner(u1);
        ...
        authUser1 = buildAuth("g-rsq-" + ts, "rsq-" + ts + "@ex.com");

        User u2 = new User(); u2.setEmail("rsq2-" + ts + "@ex.com"); u2.setStatus("ACTIVE");
        u2 = userRepository.save(u2);
        AuthAccount a2 = new AuthAccount(); a2.setUser(u2); a2.setProvider("GOOGLE");
        a2.setProviderUserId("g-rsq2-" + ts);
        authAccountRepository.save(a2);
        Workspace ws2 = new Workspace();
        ws2.setName("W2-" + ts); ws2.setSlug("ws-rsq-2-" + ts); ws2.setOwner(u2);
        ...
        authUser2 = buildAuth("g-rsq2-" + ts, "rsq2-" + ts + "@ex.com");
    }
    ...
    void PUT_status_nominal_returns200() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/case-files/" + caseFileWs1.getId() + "/risques/status")
                        .with(authentication(authUser1))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.statut").value("VALIDE"))
                .andExpect(jsonPath("$.risqueLibelleNormalise").value("harcèlement moral subi"));
    }
    ...
    void PUT_status_idempotent_2puts_oneEntry() throws Exception {
        ...
        var all = risqueStatusRepository.findByCaseFileId(caseFileWs1.getId());
    ...
    void PUT_status_caseFileInOtherWorkspace_returns404() throws Exception {
        Map<String, String> body = Map.of(
                "risqueLibelleOriginal", "Risque",
                "statut", "VALIDE");
        mockMvc.perform(put("/api/v1/case-files/" + caseFileWs2.getId() + "/risques/status")
                        .with(authentication(authUser1))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
    ...
    void GET_alignment_noAnalysis_returnsEmptyArray() throws Exception {
        mockMvc.perform(get("/api/v1/case-files/" + caseFileWs1.getId() + "/risques-alignment")
                        .with(authentication(authUser1)))
                .andExpect(status().isOk())
    ...
    void GET_alignment_legacyAnalysisWithoutJson_returnsEmptyArray() throws Exception {
        ...
        a.setAnalysisResult("{}");
        a.setRisquesAlignmentJson(null);
    ...
    void GET_alignment_withMaterialized_returnsList() throws Exception {
        ...
        a.setRisquesAlignmentJson(
                "[{\"risqueLibelle\":\"Harcèlement moral\","
                        + "\"statut\":\"VALIDE\",\"toolIdsCibles\":[\"F-DT-12-harcelement-licenciement-nul\"]}]");
        caseAnalysisRepository.save(a);
        ...
        mockMvc.perform(get("/api/v1/case-files/" + caseFileWs1.getId() + "/risques-alignment")
                ...
                .andExpect(jsonPath("$[0].risqueLibelle").value("Harcèlement moral"))
                .andExpect(jsonPath("$[0].statut").value("VALIDE"))
                .andExpect(jsonPath("$[0].toolIdsCibles[0]").value("F-DT-12-harcelement-licenciement-nul"));
    }

    @Test
    void GET_alignment_caseFileInOtherWorkspace_returns404() throws Exception {
        mockMvc.perform(get("/api/v1/case-files/" + caseFileWs2.getId() + "/risques-alignment")
                        .with(authentication(authUser1)))
                .andExpect(status().isNotFound());
    }
    ...
    void GET_alignment_unauthenticated_returnsRedirectOrUnauthorized() throws Exception {
        int sc = mockMvc.perform(get("/api/v1/case-files/" + caseFileWs1.getId() + "/risques-alignment"))
    ...
    void PUT_status_unauthenticated_returnsRedirectOrUnauthorized() throws Exception {
        Map<String, String> body = Map.of(
                "risqueLibelleOriginal", "Risque",
                "statut", "VALIDE");
        int sc = mockMvc.perform(put("/api/v1/case-files/" + caseFileWs1.getId() + "/risques/status")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andReturn().getResponse().getStatus();
        assert sc == 401 || sc == 403 || sc == 302
