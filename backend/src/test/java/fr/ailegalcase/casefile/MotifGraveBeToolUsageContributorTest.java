...
class MotifGraveBeToolUsageContributorTest {
    private MotifGraveBeRepository repository;
    private MotifGraveBeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(MotifGraveBeRepository.class);
        contributor = new MotifGraveBeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-DT-27-motif-grave-be");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new MotifGraveBeAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-DT-27-motif-grave-be");
