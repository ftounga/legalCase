...
class VictimeTraiteToolUsageContributorTest {

    private VictimeTraiteRepository repository;
    private VictimeTraiteToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(VictimeTraiteRepository.class);
        contributor = new VictimeTraiteToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-35-victime-traite-l4251-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new VictimeTraiteAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-35-victime-traite-l4251-fr");
