...
class OqtfCategoriesToolUsageContributorTest {

    private OqtfCategoriesRepository repository;
    private OqtfCategoriesToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(OqtfCategoriesRepository.class);
        contributor = new OqtfCategoriesToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-29-oqtf-categories-l6111-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new OqtfCategoriesAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-29-oqtf-categories-l6111-fr");
