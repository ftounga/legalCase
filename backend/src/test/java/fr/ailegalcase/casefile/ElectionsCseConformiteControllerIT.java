...
/**
 * SF-212-31 : tests d'intégration des endpoints de vérification de
...
class ElectionsCseConformiteControllerIT {
    ...
    void setUp() {
        ...
        User u1 = new User(); u1.setEmail("cse-fr-" + ts + "@ex.com"); u1.setStatus("ACTIVE"); u1 = userRepository.save(u1);
        AuthAccount a1 = new AuthAccount(); a1.setUser(u1); a1.setProvider("GOOGLE"); a1.setProviderUserId("g-cse-fr-" + ts); authAccountRepository.save(a1);
        Workspace ws1 = new Workspace(); ws1.setName("CSE FR " + ts); ws1.setSlug("ws-cse-fr-" + ts); ws1.setOwner(u1);
        ...
        travailFrCf = new CaseFile(); travailFrCf.setTitle("CSE FR " + ts); travailFrCf.setWorkspace(ws1); travailFrCf.setCreatedBy(u1);
        travailFrCf.setLegalDomain("DROIT_DU_TRAVAIL"); travailFrCf.setStatus("OPEN"); travailFrCf = caseFileRepository.save(travailFrCf);
        authFr = buildAuth("g-cse-fr-" + ts, "cse-fr-" + ts + "@ex.com");

        User u2 = new User(); u2.setEmail("cse-be-" + ts + "@ex.com"); u2.setStatus("ACTIVE"); u2 = userRepository.save(u2);
        AuthAccount a2 = new AuthAccount(); a2.setUser(u2); a2.setProvider("GOOGLE"); a2.setProviderUserId("g-cse-be-" + ts); authAccountRepository.save(a2);
        Workspace ws2 = new Workspace(); ws2.setName("CSE BE " + ts); ws2.setSlug("ws-cse-be-" + ts); ws2.setOwner(u2);
        ...
        travailBeCf = new CaseFile(); travailBeCf.setTitle("CSE BE " + ts); travailBeCf.setWorkspace(ws2); travailBeCf.setCreatedBy(u2);
        travailBeCf.setLegalDomain("DROIT_DU_TRAVAIL"); travailBeCf.setStatus("OPEN"); travailBeCf = caseFileRepository.save(travailBeCf);
        authBe = buildAuth("g-cse-be-" + ts, "cse-be-" + ts + "@ex.com");

        User u3 = new User(); u3.setEmail("cse-o-" + ts + "@ex.com"); u3.setStatus("ACTIVE"); u3 = userRepository.save(u3);
        AuthAccount a3 = new AuthAccount(); a3.setUser(u3); a3.setProvider("GOOGLE"); a3.setProviderUserId("g-cse-o-" + ts); authAccountRepository.save(a3);
        Workspace ws3 = new Workspace(); ws3.setName("CSEO " + ts); ws3.setSlug("ws-cse-o-" + ts); ws3.setOwner(u3);
        ...
        immCf = new CaseFile(); immCf.setTitle("CSEO " + ts); immCf.setWorkspace(ws3); immCf.setCreatedBy(u3);
        immCf.setLegalDomain("DROIT_IMMIGRATION"); immCf.setStatus("OPEN"); immCf = caseFileRepository.save(immCf);
        authOther = buildAuth("g-cse-o-" + ts, "cse-o-" + ts + "@ex.com");
    }

    // ── Nominal ─────────────────────────────────────────────────────────────

    @Test
    void POST_baselineConforme_returns200_conforme() throws Exception {
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(baselineConforme())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.analyseElectionsCse").value("CONFORME"))
                .andExpect(jsonPath("$.delaiContestationJours").value(15))
                .andExpect(jsonPath("$.country").value("FRANCE"))
                .andExpect(jsonPath("$.calculatedAt").exists());
    }

    @Test
    void POST_papAbsent_returns200_irregulariteMajeure() throws Exception {
        Map<String, Object> body = baselineConforme();
        body.put("papNegocieAvecOS", false);
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.analyseElectionsCse").value("IRREGULARITE_MAJEURE"));
    }
    ...
    void POST_recalcul_ecraseResultatPrecedent() throws Exception {
        ...
        Map<String, Object> body2 = baselineConforme();
        body2.put("collegesConformes", false);
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
    ...
    void GET_apresPost_returnsSnapshot() throws Exception {
        ...
        mockMvc.perform(get(url(travailFrCf.getId())).with(authentication(authFr)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.country").value("FRANCE"))
                .andExpect(jsonPath("$.analyseElectionsCse").exists())
                .andExpect(jsonPath("$.delaiContestationJours").value(15));
    }
    ...
    private String url(UUID caseFileId) {
        return "/api/v1/case-files/" + caseFileId + "/elections-cse-conformite";
    }

    /** Body baseline conforme : 25 salariés, PAP négocié, OS invitées, collèges conformes. */
    private Map<String, Object> baselineConforme() {
        Map<String, Object> m = new LinkedHashMap<>();
        m.put("effectifEntreprise", 25);
        m.put("papNegocieAvecOS", true);
        m.put("delaiInvitationOSRespectee", true);
        m.put("collegesConformes", true);
        m.put("resultatsContestes", false);
        m.put("dateElection", null);
        m.put("motifContestation", null);
        return m;
    }
