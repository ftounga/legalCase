...
class AdoptionIntraFrToolUsageContributorTest {
    private AdoptionIntraFrRepository repository;
    private AdoptionIntraFrToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(AdoptionIntraFrRepository.class);
        contributor = new AdoptionIntraFrToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-FA-ADOPTION-INTRA");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new AdoptionIntraFrAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-FA-ADOPTION-INTRA");
