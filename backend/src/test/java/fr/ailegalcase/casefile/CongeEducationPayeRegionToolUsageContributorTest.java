...
/**
 * F-JU-03 / SF-219-11 — vérifie la détection d'usage de l'outil
 * conge-education-paye-region via la présence d'une analyse persistée.
 */
class CongeEducationPayeRegionToolUsageContributorTest {

    private CongeEducationPayeRegionAnalysisRepository repository;
    private CongeEducationPayeRegionToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(CongeEducationPayeRegionAnalysisRepository.class);
        contributor = new CongeEducationPayeRegionToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("conge-education-paye-region");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new CongeEducationPayeRegionAnalysis()));
