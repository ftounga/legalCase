...
/**
 * F-JU-03 / SF-219-27 — vérifie la détection d'usage de l'outil
 * inastri-statut-travailleur-independant via la présence d'une
 * analyse persistée.
 */
class InastriStatutTravailleurIndependantToolUsageContributorTest {

    private InastriStatutTravailleurIndependantAnalysisRepository repository;
    private InastriStatutTravailleurIndependantToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(
                InastriStatutTravailleurIndependantAnalysisRepository.class);
        contributor =
                new InastriStatutTravailleurIndependantToolUsageContributor(
                        repository);
    }
    ...
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("inastri-statut-travailleur-independant");
    }
