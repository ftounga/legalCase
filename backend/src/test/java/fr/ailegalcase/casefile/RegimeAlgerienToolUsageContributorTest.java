...
class RegimeAlgerienToolUsageContributorTest {
    private RegimeAlgerienRepository repository;
    private RegimeAlgerienToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(RegimeAlgerienRepository.class);
        contributor = new RegimeAlgerienToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-17-regime-algerien");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new RegimeAlgerienAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-17-regime-algerien");
