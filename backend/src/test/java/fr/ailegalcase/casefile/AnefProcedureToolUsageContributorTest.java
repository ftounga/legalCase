...
class AnefProcedureToolUsageContributorTest {

    private AnefProcedureRepository repository;
    private AnefProcedureToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(AnefProcedureRepository.class);
        contributor = new AnefProcedureToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-37-anef-procedure-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new AnefProcedureAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-37-anef-procedure-fr");
