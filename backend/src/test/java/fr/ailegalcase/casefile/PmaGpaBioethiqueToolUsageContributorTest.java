...
class PmaGpaBioethiqueToolUsageContributorTest {
    private PmaGpaBioethiqueRepository repository;
    private PmaGpaBioethiqueToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(PmaGpaBioethiqueRepository.class);
        contributor = new PmaGpaBioethiqueToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-FA-27-pma-gpa");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new PmaGpaBioethiqueAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-FA-27-pma-gpa");
