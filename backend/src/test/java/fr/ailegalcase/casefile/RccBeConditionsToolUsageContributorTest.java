...
class RccBeConditionsToolUsageContributorTest {
    private RccBeConditionsRepository repository;
    private RccBeConditionsToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(RccBeConditionsRepository.class);
        contributor = new RccBeConditionsToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("rcc-be-conditions");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new RccBeConditionsAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("rcc-be-conditions");
