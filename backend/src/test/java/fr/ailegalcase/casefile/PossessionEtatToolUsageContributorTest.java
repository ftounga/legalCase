...
class PossessionEtatToolUsageContributorTest {
    private PossessionEtatRepository repository;
    private PossessionEtatToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(PossessionEtatRepository.class);
        contributor = new PossessionEtatToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-FA-18-possession-etat");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new PossessionEtatAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-FA-18-possession-etat");
