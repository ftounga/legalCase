...
class AjCndaToolUsageContributorTest {

    private AjCndaRepository repository;
    private AjCndaToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(AjCndaRepository.class);
        contributor = new AjCndaToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-34-aj-cnda-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new AjCndaAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-34-aj-cnda-fr");
