...
class DublinRecoursToolUsageContributorTest {

    private DublinRecoursRepository repository;
    private DublinRecoursToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(DublinRecoursRepository.class);
        contributor = new DublinRecoursToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-22-dublin-recours-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new DublinRecoursAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-22-dublin-recours-fr");
