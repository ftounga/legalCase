...
class AsileAvanceToolUsageContributorTest {
    private AsileAvanceRepository repository;
    private AsileAvanceToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(AsileAvanceRepository.class);
        contributor = new AsileAvanceToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-12-asile-avance");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new AsileAvanceAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-12-asile-avance");
