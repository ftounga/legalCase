...
class DivorceDdiBeToolUsageContributorTest {
    private DivorceDdiBeRepository repository;
    private DivorceDdiBeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(DivorceDdiBeRepository.class);
        contributor = new DivorceDdiBeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("divorce-ddi-3voies-be");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new DivorceDdiBeAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("divorce-ddi-3voies-be");
