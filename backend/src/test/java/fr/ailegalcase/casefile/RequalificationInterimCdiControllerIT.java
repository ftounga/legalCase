...
class RequalificationInterimCdiControllerIT {
    ...
    void setUp() {
        ...
        User u1 = new User(); u1.setEmail("rqint-" + ts + "@ex.com"); u1.setStatus("ACTIVE"); u1 = userRepository.save(u1);
        AuthAccount a1 = new AuthAccount(); a1.setUser(u1); a1.setProvider("GOOGLE"); a1.setProviderUserId("g-rqint-" + ts); authAccountRepository.save(a1);
        Workspace ws1 = new Workspace(); ws1.setName("RQINT " + ts); ws1.setSlug("ws-rqint-" + ts); ws1.setOwner(u1);
        ...
        travailCf = new CaseFile(); travailCf.setTitle("RQINT " + ts); travailCf.setWorkspace(ws1); travailCf.setCreatedBy(u1);
        travailCf.setLegalDomain("DROIT_DU_TRAVAIL"); travailCf.setStatus("OPEN"); travailCf = caseFileRepository.save(travailCf);
        auth = buildAuth("g-rqint-" + ts, "rqint-" + ts + "@ex.com");

        User u2 = new User(); u2.setEmail("rqint-o-" + ts + "@ex.com"); u2.setStatus("ACTIVE"); u2 = userRepository.save(u2);
        AuthAccount a2 = new AuthAccount(); a2.setUser(u2); a2.setProvider("GOOGLE"); a2.setProviderUserId("g-rqint-o-" + ts); authAccountRepository.save(a2);
        Workspace ws2 = new Workspace(); ws2.setName("RQINTO " + ts); ws2.setSlug("ws-rqint-o-" + ts); ws2.setOwner(u2);
        ...
        immCf = new CaseFile(); immCf.setTitle("RQINTO " + ts); immCf.setWorkspace(ws2); immCf.setCreatedBy(u2);
        immCf.setLegalDomain("DROIT_IMMIGRATION"); immCf.setStatus("OPEN"); immCf = caseFileRepository.save(immCf);
        authOther = buildAuth("g-rqint-o-" + ts, "rqint-o-" + ts + "@ex.com");
    }

    private Map<String, Object> nominalRequestBody() {
        Map<String, Object> body = new HashMap<>();
        body.put("motifInterimInvoque", "ACCROISSEMENT_TEMPORAIRE");
        body.put("motifInterdit", false);
        body.put("motifInterditType", null);
        body.put("successionMissions", List.of(
                Map.of("dateDebut", "2024-01-01", "dateFin", "2024-06-30", "motif", "ACCROISSEMENT", "entrepriseUtilisatrice", "ENT_X"),
                Map.of("dateDebut", "2024-08-01", "dateFin", "2024-12-31", "motif", "REMPLACEMENT", "entrepriseUtilisatrice", "ENT_X")
        ));
        body.put("delaiCarenceRespecte", false);
        body.put("dureeMissionsTotaleMois", 12);
        body.put("salaireMensuelBrutEur", 2500.00);
        body.put("dateFinDerniereMission", "2024-12-31");
        body.put("memeEntrepriseUtilisatrice", true);
        return body;
    }

    @Test
    void POST_nominal_returns200() throws Exception {
        mockMvc.perform(post("/api/v1/case-files/" + travailCf.getId() + "/requalification-interim-cdi")
                ...
                // 0 (motif licite) + 0 (succession 2 < 3) + 20 (carence) + 0 = 20
                ...
                .andExpect(jsonPath("$.indemniteFinMissionInterimEur").value(3000.00))
                .andExpect(jsonPath("$.totalDommagesIndemniteEur").value(5500.00))
                .andExpect(jsonPath("$.country").value("FRANCE"));
    }

    @Test
    void POST_motifInterditTrue_returnsScore70() throws Exception {
    ...
    void POST_immigrationCaseFile_returns400() throws Exception {
        mockMvc.perform(post("/api/v1/case-files/" + immCf.getId() + "/requalification-interim-cdi")
    ...
    void GET_afterPost_returnsPersisted() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/case-files/" + travailCf.getId() + "/requalification-interim-cdi")
