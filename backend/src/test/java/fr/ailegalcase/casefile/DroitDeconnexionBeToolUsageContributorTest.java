...
/**
 * F-JU-03 / SF-219-19 — vérifie la détection d'usage de l'outil
 * droit-deconnexion-be via la présence d'une analyse persistée.
 */
class DroitDeconnexionBeToolUsageContributorTest {

    private DroitDeconnexionBeAnalysisRepository repository;
    private DroitDeconnexionBeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(DroitDeconnexionBeAnalysisRepository.class);
        contributor =
                new DroitDeconnexionBeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("droit-deconnexion-be");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new DroitDeconnexionBeAnalysis()));
