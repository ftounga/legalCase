...
/**
 * F-JU-03 / SF-219-07 — vérifie la détection d'usage de l'outil
 * licenciement-be-collectif-renault via la présence d'une analyse
 * persistée.
 */
class LicenciementBeCollectifRenaultToolUsageContributorTest {

    private LicenciementBeCollectifRenaultAnalysisRepository repository;
    private LicenciementBeCollectifRenaultToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(LicenciementBeCollectifRenaultAnalysisRepository.class);
        contributor = new LicenciementBeCollectifRenaultToolUsageContributor(
                repository);
    }
    ...
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("licenciement-be-collectif-renault");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new LicenciementBeCollectifRenaultAnalysis()));
