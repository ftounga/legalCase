...
class ItfJudiciaireToolUsageContributorTest {

    private ItfJudiciaireRepository repository;
    private ItfJudiciaireToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(ItfJudiciaireRepository.class);
        contributor = new ItfJudiciaireToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-43-itf-judiciaire-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new ItfJudiciaireAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-43-itf-judiciaire-fr");
