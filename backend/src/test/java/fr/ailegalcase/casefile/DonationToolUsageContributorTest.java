...
class DonationToolUsageContributorTest {
    private DonationRepository repository;
    private DonationToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(DonationRepository.class);
        contributor = new DonationToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-FA-24-donation");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new DonationAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-FA-24-donation");
