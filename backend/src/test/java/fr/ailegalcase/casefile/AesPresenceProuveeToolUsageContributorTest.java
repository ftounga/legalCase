...
class AesPresenceProuveeToolUsageContributorTest {

    private AesPresenceProuveeRepository repository;
    private AesPresenceProuveeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(AesPresenceProuveeRepository.class);
        contributor = new AesPresenceProuveeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-30-aes-presence-prouvee-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new AesPresenceProuveeAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-30-aes-presence-prouvee-fr");
