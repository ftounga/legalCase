...
/**
 * F-JU-03 / SF-219-09 — vérifie la détection d'usage de l'outil
 * elections-sociales-be via la présence d'une analyse persistée.
 */
class ElectionsSocialesBeToolUsageContributorTest {

    private ElectionsSocialesBeAnalysisRepository repository;
    private ElectionsSocialesBeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(ElectionsSocialesBeAnalysisRepository.class);
        contributor = new ElectionsSocialesBeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("elections-sociales-be");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new ElectionsSocialesBeAnalysis()));
