...
class VlsTsValidationToolUsageContributorTest {

    private VlsTsValidationRepository repository;
    private VlsTsValidationToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(VlsTsValidationRepository.class);
        contributor = new VlsTsValidationToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-28-vls-ts-validation-ofii-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new VlsTsValidationAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-28-vls-ts-validation-ofii-fr");
