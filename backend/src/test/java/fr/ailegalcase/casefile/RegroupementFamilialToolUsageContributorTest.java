...
class RegroupementFamilialToolUsageContributorTest {

    private RegroupementFamilialRepository repository;
    private RegroupementFamilialToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(RegroupementFamilialRepository.class);
        contributor = new RegroupementFamilialToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-26-regroupement-familial-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new RegroupementFamilialAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-26-regroupement-familial-fr");
