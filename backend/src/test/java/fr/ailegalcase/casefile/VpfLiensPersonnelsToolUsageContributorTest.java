...
class VpfLiensPersonnelsToolUsageContributorTest {

    private VpfLiensPersonnelsRepository repository;
    private VpfLiensPersonnelsToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(VpfLiensPersonnelsRepository.class);
        contributor = new VpfLiensPersonnelsToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-27-vpf-liens-personnels-l42323-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new VpfLiensPersonnelsAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-27-vpf-liens-personnels-l42323-fr");
