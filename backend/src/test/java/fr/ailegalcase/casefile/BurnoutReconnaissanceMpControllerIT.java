...
/**
 * SF-212-27 : tests d'intégration des endpoints d'évaluation des chances
...
class BurnoutReconnaissanceMpControllerIT {
    ...
    void setUp() {
        ...
        User u1 = new User(); u1.setEmail("bo-fr-" + ts + "@ex.com"); u1.setStatus("ACTIVE"); u1 = userRepository.save(u1);
        AuthAccount a1 = new AuthAccount(); a1.setUser(u1); a1.setProvider("GOOGLE"); a1.setProviderUserId("g-bo-fr-" + ts); authAccountRepository.save(a1);
        Workspace ws1 = new Workspace(); ws1.setName("BO FR " + ts); ws1.setSlug("ws-bo-fr-" + ts); ws1.setOwner(u1);
        ...
        travailFrCf = new CaseFile(); travailFrCf.setTitle("BO FR " + ts); travailFrCf.setWorkspace(ws1); travailFrCf.setCreatedBy(u1);
        travailFrCf.setLegalDomain("DROIT_DU_TRAVAIL"); travailFrCf.setStatus("OPEN"); travailFrCf = caseFileRepository.save(travailFrCf);
        authFr = buildAuth("g-bo-fr-" + ts, "bo-fr-" + ts + "@ex.com");

        User u2 = new User(); u2.setEmail("bo-be-" + ts + "@ex.com"); u2.setStatus("ACTIVE"); u2 = userRepository.save(u2);
        AuthAccount a2 = new AuthAccount(); a2.setUser(u2); a2.setProvider("GOOGLE"); a2.setProviderUserId("g-bo-be-" + ts); authAccountRepository.save(a2);
        Workspace ws2 = new Workspace(); ws2.setName("BO BE " + ts); ws2.setSlug("ws-bo-be-" + ts); ws2.setOwner(u2);
        ...
        travailBeCf = new CaseFile(); travailBeCf.setTitle("BO BE " + ts); travailBeCf.setWorkspace(ws2); travailBeCf.setCreatedBy(u2);
        travailBeCf.setLegalDomain("DROIT_DU_TRAVAIL"); travailBeCf.setStatus("OPEN"); travailBeCf = caseFileRepository.save(travailBeCf);
        authBe = buildAuth("g-bo-be-" + ts, "bo-be-" + ts + "@ex.com");

        User u3 = new User(); u3.setEmail("bo-o-" + ts + "@ex.com"); u3.setStatus("ACTIVE"); u3 = userRepository.save(u3);
        AuthAccount a3 = new AuthAccount(); a3.setUser(u3); a3.setProvider("GOOGLE"); a3.setProviderUserId("g-bo-o-" + ts); authAccountRepository.save(a3);
        Workspace ws3 = new Workspace(); ws3.setName("BOO " + ts); ws3.setSlug("ws-bo-o-" + ts); ws3.setOwner(u3);
        ...
        immCf = new CaseFile(); immCf.setTitle("BOO " + ts); immCf.setWorkspace(ws3); immCf.setCreatedBy(u3);
        immCf.setLegalDomain("DROIT_IMMIGRATION"); immCf.setStatus("OPEN"); immCf = caseFileRepository.save(immCf);
        authOther = buildAuth("g-bo-o-" + ts, "bo-o-" + ts + "@ex.com");
    }

    // ── Nominal ─────────────────────────────────────────────────────────────

    @Test
    void POST_dossierSolide_returns200_chancesImportantes() throws Exception {
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(baseline())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.analyseChancesCRRMP").value("CHANCES_IMPORTANTES"))
                .andExpect(jsonPath("$.conditionIPPRemplie").value(true))
                .andExpect(jsonPath("$.alerteIPPInsuffisante").value(false))
                .andExpect(jsonPath("$.delaiInstructionMois").exists())
                .andExpect(jsonPath("$.country").value("FRANCE"))
                .andExpect(jsonPath("$.calculatedAt").exists());
    }

    @Test
    void POST_tauxIPPInferieurA25_returns200_alerteIPP() throws Exception {
        Map<String, Object> body = baseline();
        body.put("tauxIPPEstime", 15.0);
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.analyseChancesCRRMP").value("CHANCES_FAIBLES"))
                .andExpect(jsonPath("$.alerteIPPInsuffisante").value(true))
                .andExpect(jsonPath("$.conditionIPPRemplie").value(false));
    }
    ...
    void GET_apresPost_returnsSnapshot() throws Exception {
        ...
        mockMvc.perform(get(url(travailFrCf.getId())).with(authentication(authFr)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.country").value("FRANCE"))
                .andExpect(jsonPath("$.analyseChancesCRRMP").exists());
    }
    ...
    private String url(UUID caseFileId) {
        return "/api/v1/case-files/" + caseFileId + "/burnout-reconnaissance-mp";
    }

    /** Body baseline — dossier solide : diagnostic + IPP 30 % + lien causal + surcharge. */
    private Map<String, Object> baseline() {
        Map<String, Object> m = new LinkedHashMap<>();
        m.put("diagnosticBurnoutPose", true);
        m.put("tauxIPPEstime", 30.0);
        m.put("anneesExpositionProfessionnelle", 7);
        m.put("surchargeChargeDocumentee", true);
        m.put("manquementsSecuriteDocumentes", true);
        m.put("harcelementConcomitant", false);
        m.put("arretsMaladieMultiples", true);
        m.put("lienCausalDirectEtabli", true);
        return m;
    }
