...
class DelegationApFrToolUsageContributorTest {
    private DelegationApFrRepository repository;
    private DelegationApFrToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(DelegationApFrRepository.class);
        contributor = new DelegationApFrToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-FA-XX-delegation-ap");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new DelegationApFrAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-FA-XX-delegation-ap");
