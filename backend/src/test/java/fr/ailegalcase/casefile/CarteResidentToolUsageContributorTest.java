...
class CarteResidentToolUsageContributorTest {

    private CarteResidentRepository repository;
    private CarteResidentToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(CarteResidentRepository.class);
        contributor = new CarteResidentToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-36-carte-resident-l4261-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new CarteResidentAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-36-carte-resident-l4261-fr");
