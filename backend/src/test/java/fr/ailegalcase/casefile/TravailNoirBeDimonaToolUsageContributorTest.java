...
/**
 * F-JU-03 / SF-219-26 — vérifie la détection d'usage de l'outil
 * travail-noir-be-dimona via la présence d'une analyse persistée.
 */
class TravailNoirBeDimonaToolUsageContributorTest {

    private TravailNoirBeDimonaAnalysisRepository repository;
    private TravailNoirBeDimonaToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(TravailNoirBeDimonaAnalysisRepository.class);
        contributor =
                new TravailNoirBeDimonaToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("travail-noir-be-dimona");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new TravailNoirBeDimonaAnalysis()));
