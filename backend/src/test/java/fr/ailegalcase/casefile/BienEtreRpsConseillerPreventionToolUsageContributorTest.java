...
/**
 * F-JU-03 / SF-219-30 — vérifie la détection d'usage de l'outil
 * bien-etre-rps-conseiller-prevention via la présence d'une analyse
 * persistée.
 */
class BienEtreRpsConseillerPreventionToolUsageContributorTest {

    private BienEtreRpsConseillerPreventionAnalysisRepository repository;
    private BienEtreRpsConseillerPreventionToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(
                BienEtreRpsConseillerPreventionAnalysisRepository.class);
        contributor =
                new BienEtreRpsConseillerPreventionToolUsageContributor(
                        repository);
    }
    ...
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("bien-etre-rps-conseiller-prevention");
    }
