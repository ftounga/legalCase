...
class RetraitTitreFraudeToolUsageContributorTest {

    private RetraitTitreFraudeRepository repository;
    private RetraitTitreFraudeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(RetraitTitreFraudeRepository.class);
        contributor = new RetraitTitreFraudeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-45-retrait-titre-fraude-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new RetraitTitreFraudeAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-45-retrait-titre-fraude-fr");
