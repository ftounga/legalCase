...
/**
 * F-JU-03 / SF-219-24 — vérifie la détection d'usage de l'outil
 * code-penal-social-be via la présence d'une analyse persistée.
 */
class CodePenalSocialBeToolUsageContributorTest {

    private CodePenalSocialBeAnalysisRepository repository;
    private CodePenalSocialBeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(CodePenalSocialBeAnalysisRepository.class);
        contributor =
                new CodePenalSocialBeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("code-penal-social-be");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new CodePenalSocialBeAnalysis()));
