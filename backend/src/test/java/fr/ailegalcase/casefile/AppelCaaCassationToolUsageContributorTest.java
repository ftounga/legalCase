...
class AppelCaaCassationToolUsageContributorTest {

    private AppelCaaCassationRepository repository;
    private AppelCaaCassationToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(AppelCaaCassationRepository.class);
        contributor = new AppelCaaCassationToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-IM-41-appel-caa-cassation-ce-fr");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        ...
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new AppelCaaCassationAnalysis()));

        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);

        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-IM-41-appel-caa-cassation-ce-fr");
