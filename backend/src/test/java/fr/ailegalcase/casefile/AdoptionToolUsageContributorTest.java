...
class AdoptionToolUsageContributorTest {
    private AdoptionRepository repository;
    private AdoptionToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(AdoptionRepository.class);
        contributor = new AdoptionToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId()).isEqualTo("F-FA-18-adoption");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId)).thenReturn(Optional.of(new AdoptionAnalysis()));
        Optional<ToolUsage> result = contributor.detectUsage(caseFileId);
        assertThat(result).isPresent();
        assertThat(result.get().toolId()).isEqualTo("F-FA-18-adoption");
