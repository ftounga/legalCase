...
/**
 * F-JU-03 / SF-219-22 — vérifie la détection d'usage de l'outil
 * egalite-femmes-hommes-be via la présence d'une analyse persistée.
 */
class EgaliteFemmesHommesBeToolUsageContributorTest {

    private EgaliteFemmesHommesBeAnalysisRepository repository;
    private EgaliteFemmesHommesBeToolUsageContributor contributor;

    @BeforeEach
    void setUp() {
        repository = mock(EgaliteFemmesHommesBeAnalysisRepository.class);
        contributor =
                new EgaliteFemmesHommesBeToolUsageContributor(repository);
    }

    @Test
    void toolId_returnsExpectedConstant() {
        assertThat(contributor.toolId())
                .isEqualTo("egalite-femmes-hommes-be");
    }
    ...
    void detectUsage_returnsToolUsage_whenAnalysisExists() {
        UUID caseFileId = UUID.randomUUID();
        when(repository.findByCaseFileId(caseFileId))
                .thenReturn(Optional.of(new EgaliteFemmesHommesBeAnalysis()));
