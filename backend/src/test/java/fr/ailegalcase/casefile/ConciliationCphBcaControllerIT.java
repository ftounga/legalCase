...
/**
 * SF-212-37 : tests d'intégration des endpoints de préparation de la
 * conciliation CPH BCA
...
class ConciliationCphBcaControllerIT {
    ...
    void setUp() {
        ...
        User u1 = new User(); u1.setEmail("cph-fr-" + ts + "@ex.com"); u1.setStatus("ACTIVE"); u1 = userRepository.save(u1);
        AuthAccount a1 = new AuthAccount(); a1.setUser(u1); a1.setProvider("GOOGLE"); a1.setProviderUserId("g-cph-fr-" + ts); authAccountRepository.save(a1);
        Workspace ws1 = new Workspace(); ws1.setName("CPH FR " + ts); ws1.setSlug("ws-cph-fr-" + ts); ws1.setOwner(u1);
        ...
        travailFrCf = new CaseFile(); travailFrCf.setTitle("CPH FR " + ts); travailFrCf.setWorkspace(ws1); travailFrCf.setCreatedBy(u1);
        travailFrCf.setLegalDomain("DROIT_DU_TRAVAIL"); travailFrCf.setStatus("OPEN"); travailFrCf = caseFileRepository.save(travailFrCf);
        authFr = buildAuth("g-cph-fr-" + ts, "cph-fr-" + ts + "@ex.com");

        User u2 = new User(); u2.setEmail("cph-be-" + ts + "@ex.com"); u2.setStatus("ACTIVE"); u2 = userRepository.save(u2);
        AuthAccount a2 = new AuthAccount(); a2.setUser(u2); a2.setProvider("GOOGLE"); a2.setProviderUserId("g-cph-be-" + ts); authAccountRepository.save(a2);
        Workspace ws2 = new Workspace(); ws2.setName("CPH BE " + ts); ws2.setSlug("ws-cph-be-" + ts); ws2.setOwner(u2);
        ...
        travailBeCf = new CaseFile(); travailBeCf.setTitle("CPH BE " + ts); travailBeCf.setWorkspace(ws2); travailBeCf.setCreatedBy(u2);
        travailBeCf.setLegalDomain("DROIT_DU_TRAVAIL"); travailBeCf.setStatus("OPEN"); travailBeCf = caseFileRepository.save(travailBeCf);
        authBe = buildAuth("g-cph-be-" + ts, "cph-be-" + ts + "@ex.com");

        User u3 = new User(); u3.setEmail("cph-o-" + ts + "@ex.com"); u3.setStatus("ACTIVE"); u3 = userRepository.save(u3);
        AuthAccount a3 = new AuthAccount(); a3.setUser(u3); a3.setProvider("GOOGLE"); a3.setProviderUserId("g-cph-o-" + ts); authAccountRepository.save(a3);
        Workspace ws3 = new Workspace(); ws3.setName("CPHO " + ts); ws3.setSlug("ws-cph-o-" + ts); ws3.setOwner(u3);
        ...
        immCf = new CaseFile(); immCf.setTitle("CPHO " + ts); immCf.setWorkspace(ws3); immCf.setCreatedBy(u3);
        immCf.setLegalDomain("DROIT_IMMIGRATION"); immCf.setStatus("OPEN"); immCf = caseFileRepository.save(immCf);
        authOther = buildAuth("g-cph-o-" + ts, "cph-o-" + ts + "@ex.com");
    }

    // ── Nominal ─────────────────────────────────────────────────────────────

    @Test
    void POST_baseline_returns200_palierEtMontantCalcules() throws Exception {
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(baseline())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.palierBcaApplicable").exists())
                .andExpect(jsonPath("$.montantMinimumBcaEuros").value(7500.0))
                .andExpect(jsonPath("$.country").value("FRANCE"))
                .andExpect(jsonPath("$.calculatedAt").exists())
                .andExpect(jsonPath("$.checklistBco").isArray());
    }

    @Test
    void POST_anciennete12Ans_palier4Mois() throws Exception {
        Map<String, Object> body = baseline();
        body.put("ancienneteMois", 12 * 12);
        body.put("salaireMensuelBrutEuros", 3000.0);
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.palierBcaApplicable").value("8 à 15 ans : 4 mois"))
                .andExpect(jsonPath("$.montantMinimumBcaEuros").value(12000.0));
    }
    ...
    void POST_recalcul_ecraseResultatPrecedent() throws Exception {
        ...
        body2.put("ancienneteMois", 30 * 12);
        body2.put("salaireMensuelBrutEuros", 4000.0);
        mockMvc.perform(post(url(travailFrCf.getId()))
                        .with(authentication(authFr)).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body2)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.palierBcaApplicable").value("≥ 25 ans : 14 mois"))
                .andExpect(jsonPath("$.montantMinimumBcaEuros").value(56000.0));
    ...
    void GET_apresPost_returnsSnapshot() throws Exception {
        ...
        mockMvc.perform(get(url(travailFrCf.getId())).with(authentication(authFr)))
                ...
                .andExpect(jsonPath("$.country").value("FRANCE"))
                .andExpect(jsonPath("$.palierBcaApplicable").exists())
                .andExpect(jsonPath("$.ancienneteMois").value(36));
    }
    ...
    private String url(UUID caseFileId) {
        return "/api/v1/case-files/" + caseFileId + "/conciliation-cph-bca";
    }

    /** Body baseline : 36 mois (3 ans → palier 2 à 8 ans : 3 mois), salaire 2 500 €, demandes 30 000 €. */
    private Map<String, Object> baseline() {
        Map<String, Object> m = new LinkedHashMap<>();
        m.put("ancienneteMois", 36);
        m.put("salaireMensuelBrutEuros", 2500.0);
        m.put("montantDemandesEuros", 30000.0);
        m.put("opportuniteConciliation", "FAVORABLE");
        return m;
    }
