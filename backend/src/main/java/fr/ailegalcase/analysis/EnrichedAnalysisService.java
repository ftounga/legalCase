...
public class EnrichedAnalysisService {
    ...
    static final String SYSTEM_PROMPT_TEMPLATE = """
            ...
            RÈGLE CRITIQUE "type_rupture" EN MODE ENRICHI : le type_rupture de la synthèse précédente est la BASELINE à préserver. Tu ne dois le CHANGER QUE si un nouveau document signé/notifié dans le dossier contredit le type précédent. Les réponses Q&A de l'avocat, le chat et les checks procéduraux N'AUTORISENT PAS à retourner le type_rupture (ils n'ajoutent pas de fait factuel sur la nature de la rupture). Exemple : si la synthèse précédente a classifié "RUPTURE_CONVENTIONNELLE" sur la base d'une convention signée, tu la conserves même si les réponses Q&A mentionnent "licenciement" (car le dossier vise la requalification — ce n'est pas un changement de mécanisme).
            ...
            Le champ "travail_extracted_data" doit être préservé et actualisé depuis la synthèse initiale. Pour un dossier de droit du travail, ré-émets l'objet complet avec les champs : "convention_collective", "date_entree", "salaire_brut_mensuel", "type_contrat", "poste", "motif_licenciement", "date_licenciement", "conges_contractuels", "prime_anciennete_contractuelle", "nom_salarie", "prenom_salarie", "adresse_salarie", "nom_employeur", "adresse_employeur", "siret_employeur" (FR uniquement, chiffres), "bce_employeur" (BE uniquement, chiffres), "representant_employeur". Utilise null pour les champs non extractibles. NE JAMAIS INVENTER un SIRET/BCE/adresse. Pour un dossier hors droit du travail, utilise "travail_extracted_data": null.
