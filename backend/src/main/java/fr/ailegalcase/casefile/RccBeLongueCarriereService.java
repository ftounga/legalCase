...
/**
 * SF-219-02 : service orchestrant l'analyse RCC BE longue carrière
 * (Loi 26/12/2013 + CCT n° 17 + AR 03/05/2007 art. 3 — conditions
 * 59 ans / 40 ans de carrière).
 *
 ...
 *
 * <p>Le validateur sous-jacent ({@link RccBeLongueCarriereValidator}) est
 * une fonction pure indépendante du contexte HTTP / persistance.</p>
 */
@Service
public class RccBeLongueCarriereService {
    ...
    public RccBeLongueCarriereService(
    ...
    public RccBeLongueCarriereResponse analyze(
        ...
        // dossier inconnu (aucun équivalent FR du RCC longue carrière).
        if (!"BELGIQUE".equals(country)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                    "Case file not found");
        }

        RccBeLongueCarriereResult result;
        try {
            result = RccBeLongueCarriereValidator.analyze(request);
        } catch (IllegalArgumentException e) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getMessage());
        }

        RccBeLongueCarriereResponse response = toResponse(caseFileId, result);

        RccBeLongueCarriereAnalysis entity = repository.findByCaseFileId(caseFileId)
                .orElseGet(() -> {
                    RccBeLongueCarriereAnalysis a = new RccBeLongueCarriereAnalysis();
    ...
    public RccBeLongueCarriereResponse get(
        ...
        RccBeLongueCarriereAnalysis entity = repository.findByCaseFileId(caseFileId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                        "Aucune analyse RCC BE longue carrière trouvée pour "
                                + "ce dossier"));
    ...
    private RccBeLongueCarriereResponse deserialize(String json) {
        try {
            return objectMapper.readValue(json, RccBeLongueCarriereResponse.class);
        } catch (JsonProcessingException e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
    ...
    private RccBeLongueCarriereResponse toResponse(
            UUID caseFileId, RccBeLongueCarriereResult r) {
        return new RccBeLongueCarriereResponse(
                caseFileId,
                r.ageFinContrat(),
