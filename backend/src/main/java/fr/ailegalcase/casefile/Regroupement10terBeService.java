...
/**
 * SF-215-03 : service applicatif de l'outil regroupement familial 10ter BE
 * (Loi 15/12/1980 art. 10 et 10ter + AR 17/05/2007).
 *
 * <p>Gates :
 * <ul>
 ...
 */
@Service
public class Regroupement10terBeService {
    ...
    public Regroupement10terBeService(
            Regroupement10terBeRepository repository,
    ...
    @Transactional
    public Regroupement10terBeResponse calculate(UUID caseFileId,
                                                 Regroupement10terBeRequest request,
                                                 OidcUser oidcUser,
                                                 Principal principal) {
        ...
        if (!"BELGIQUE".equals(country)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Regroupement 10ter BE uniquement — pour la France voir les outils "
        ...
        Regroupement10terBeResult result;
        try {
            result = Regroupement10terBeCalculator.compute(
        ...
        Regroupement10terBeAnalysis entity = repository.findByCaseFileId(caseFileId)
                .orElseGet(() -> {
                    Regroupement10terBeAnalysis a = new Regroupement10terBeAnalysis();
    ...
    public Regroupement10terBeResponse get(UUID caseFileId, OidcUser oidcUser, Principal principal) {
        User user = resolveUser(oidcUser, principal);
        resolveCaseFile(caseFileId, user);
        Regroupement10terBeAnalysis entity = repository.findByCaseFileId(caseFileId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                        "Aucune analyse regroupement 10ter BE trouvée pour ce dossier"));
    ...
    private Regroupement10terBeResult deserialize(String json) {
        try {
            return objectMapper.readValue(json, Regroupement10terBeResult.class);
        } catch (JsonProcessingException e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Erreur de désérialisation");
        }
    }

    private Regroupement10terBeResponse toResponse(UUID caseFileId, Regroupement10terBeResult r) {
        return new Regroupement10terBeResponse(
                caseFileId,
                r.lienFamilial(),
