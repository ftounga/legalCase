...
@Service
public class RuptureConvService {

    private final RuptureConvAnalysisRepository analysisRepository;
    ...
    @Transactional
    public RuptureConvResponse analyze(UUID caseFileId,
                                        RuptureConvRequest request,
                                        OidcUser oidcUser,
                                        Principal principal) {
        ...
        RuptureConvAnalysis entity = analysisRepository.findByCaseFileId(caseFileId)
                .orElseGet(() -> {
                    RuptureConvAnalysis a = new RuptureConvAnalysis();
                    a.setCaseFile(caseFile);
                    return a;
                });

        entity.setCountry(result.country());
        entity.setReponsesData(serialize(reponses));
    ...
    public RuptureConvResponse get(UUID caseFileId,
                                    OidcUser oidcUser,
                                    Principal principal) {
        User user = resolveUser(oidcUser, principal);
        ...
        RuptureConvAnalysis entity = analysisRepository.findByCaseFileId(caseFileId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                        "Aucune analyse de rupture conventionnelle trouvée pour ce dossier"));

        RuptureConvAnalysisResult result = deserialize(entity.getResultData(), RuptureConvAnalysisResult.class);
        return toResponse(caseFileId, result);
    }
    ...
    private RuptureConvResponse toResponse(UUID caseFileId, RuptureConvAnalysisResult result) {
        return new RuptureConvResponse(
                caseFileId, result.country(), result.scoreRisque(), result.verdict(),
                result.criteres().stream().map(c -> new RuptureConvResponse.CritereData(
