...
| Nouveau composant Angular décisionnel frontend (section type `<app-XXX-section>` consommant un endpoint POST/GET décisionnel + intégré au panel F-IA-04 via TOOL_REGISTRY) créé **sans scan de cohérence avec le template canonique** | REFUS — chaque nouveau composant décisionnel doit : (1) référencer explicitement le **template canonique** (composant de référence identifié dans `ai-skills/frontend-coherence-audit.md`) ; (2) passer la **checklist cohérence** : palette statut (navy/or/rouge réservé alerte critique uniquement), datepicker (convention `<input type="date">` ou `datetime-local` selon précision — pas MatDatepicker), gate `workspaceCountry` (bannière info si mismatch pays — pas masquage silencieux), `CaseDashboardRefreshService.triggerRefresh()` obligatoire après POST succès, `MatSnackBar` pour erreurs (pas d'alert/confirm), JetBrains Mono pour `baseJuridique` et `formule`, Inter pour le reste ; (3) ajouter entrée `TOOL_REGISTRY` symétrique aux autres (`inputs: (ctx) => ({ caseFileId, workspaceCountry })` si gate pays). **Motivation** : dette de convergence réelle observée fin session 2026-04-24 sur 6 composants décisionnels livrés en parallèle par agents — palette, pickers et handling workspaceCountry ont divergé (cas réel : F-IM-08 SF-04 rouge dominant vs SF-02/06 palette standard ; mix `<input type="date">` / `datetime-local`). Audit rétroactif → F-155 au backlog. Pour éviter que cela se reproduise, scan obligatoire à chaque nouveau composant frontend décisionnel. Le skill `ai-skills/frontend-coherence-audit.md` formalise la checklist et peut être invoqué pour audit périodique (tous les 5 nouveaux composants, règle similaire à "audit couverture 3 domaines"). |
