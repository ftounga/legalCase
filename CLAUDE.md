...
| Nouveau composant Angular décisionnel frontend (section type `<app-XXX-section>` consommant un endpoint POST/GET décisionnel + intégré au panel F-IA-04 via TOOL_REGISTRY) créé **sans scan de cohérence avec le template canonique** | REFUS — chaque nouveau composant décisionnel doit : (1) référencer explicitement le **template canonique** (composant de référence identifié dans `ai-skills/frontend-coherence-audit.md`) ; (2) passer la **checklist cohérence visuelle** : palette statut (navy/or/rouge réservé alerte critique uniquement), datepicker (convention `<input type="date">` ou `datetime-local` selon précision — pas MatDatepicker), gate `workspaceCountry` (bannière info si mismatch pays — pas masquage silencieux), `CaseDashboardRefreshService.triggerRefresh()` obligatoire après POST succès, `MatSnackBar` pour erreurs (pas d'alert/confirm), JetBrains Mono pour `baseJuridique` et `formule`, Inter pour le reste ; (3) implémenter le **pré-remplissage IA** (OBLIGATOIRE, FAIL si absent — pas WARN) : `@Input() aiData?` typé (`TravailExtractedData` / `ImmigrationExtractedData` / `FamilleExtractedData`) + méthode privée `prefillFromAi()` invoquée dans `ngOnInit()` ET `ngOnChanges()` (si `aiData` change avant première résolution) + signal `provenance<Field> = signal<'IA'\|null>(null)` par champ pré-rempli + badge UI à côté du champ (icône `auto_awesome` "Pré-rempli depuis l'analyse") + handler `onXxxChange()` qui remet `provenance<Field>` à `null` au changement manuel de l'avocat. Pattern de référence : `immigration-title-decision-section` (méthode `prefillFromAi()` + signals + handlers) ; (4) implémenter la **validation IA au changement F-IA-03** (OBLIGATOIRE, FAIL si absent — pas WARN) : `coherenceAlerts = computed<Partial<Record<FieldName, CoherenceAlert>>>()` qui produit une alerte par field clé quand la valeur affichée diverge des 4 sources IA disponibles (`aiData`, `procedureChecks` F-96, `aiQuestions`, `piecesManquantes`) — source `MULTI` si convergence entre plusieurs sources + directive `<app-coherence-popover-trigger>` câblée sur chaque field concerné (le popover affiche la divergence avec la source sans blocage technique) + helper partagé `CoherenceAlertBuilder` obligatoire (chemin `frontend/src/app/shared/coherence-popover/coherence-alert-builder.ts`) pour instancier les alertes de façon uniforme (pas de définition locale ad-hoc d'interface `CoherenceAlert` — source de dette de convergence). Hiérarchie de sources F-96 > Question IA > IA détection > Pièce manquante (règle F-IA-03). Pattern de référence : `immigration-title-decision-section` (`buildMotifAlert`, `buildNationaliteAlert`, `coherenceAlerts` computed, `alertsSummary` computed) ; (5) ajouter entrée `TOOL_REGISTRY` symétrique aux autres (`inputs: (ctx) => ({ caseFileId, workspaceCountry, aiData, procedureChecks, aiQuestions, piecesManquantes })` — passer toutes les sources IA nécessaires à la validation F-IA-03) ; (6) exposer le **static `getPrefillCount(input): number`** (pattern miroir de `TOOL_LABEL` / `TOOL_ICON` introduits par SF-177-03b/05/07 et étendu par SF-177-12) qui reproduit **exactement** la logique de `prefillFromAi()` — utilisé par le panel F-IA-04 pour afficher le badge `auto_awesome` "Pré-rempli par l'IA (N champs)" sur la card AVANT l'instanciation effective du composant outil. Signature : `static getPrefillCount(input: { aiData?, procedureChecks?, aiQuestions?, piecesManquantes?, triggerEvents?, workspaceCountry? }): number`. Stricte parité avec `prefillFromAi()` runtime (mêmes guards `typeof === 'string'`, mêmes mappings, mêmes conditions de pays) — divergence = bug (badge faux). Tests Jest obligatoires : 0 champs, M champs partiels, N champs cas nominal. Pattern de référence : `immigration-title-decision-section` post-SF-177-12. **Motivation** : (a) dette de convergence visuelle observée fin 2026-04-24 sur 6 composants (palette, pickers, handling country) → F-155 rétroactif ; (b) sans pré-fill IA et sans validation F-IA-03 au changement, l'outil décisionnel devient "encore un formulaire" au lieu d'un assistant — l'IA ne parle pas à l'outil, chaque outil vit en silo, et l'avocat peut saisir une valeur contradictoire avec l'analyse IA sans le voir. C'est un **bug produit**, pas une dette technique. Le skill `ai-skills/frontend-coherence-audit.md` formalise la checklist complète (§5 RÈGLE FONDAMENTALE + §6 checklist §19 items dont 4 dédiés pré-fill + validation) et peut être invoqué pour audit périodique (tous les 5 nouveaux composants). |
